...
無題でまだ保存先を持たない NoteNest / IdeaNest / ChatNest タブは、30 秒間隔の自動保存 tick で `%APPDATA%\NoteNest\drafts\` に下書き保存されます。下書き本体の `draft-*.nestsuite` は、通常の「ファイル → 開く」から手動で開けます。

SH-36a（v2.16.43）時点では、起動時の自動復元ダイアログはまだありません。ChatNest の `draft-*.state.json` sidecar には未送信入力、選択中 Speaker、編集中テキストなどが JSON として保存される場合がありますが、通常の「開く」ではアプリに反映されません。必要に応じて人が内容を確認できる補助ファイルです。自動復元は SH-36b（v2.16.44）で対応予定です。

下書きには Workspace と同等の内容が含まれるため、取り扱いには通常の保存ファイルと同じ注意が必要です。正常に保存・タブ閉鎖・アプリ終了できた場合、対応する active 下書きは削除されます。
