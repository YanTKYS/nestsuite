...
| `ProjectLifecycleService.TryAutoSave()` | 現行（サービス層） | 「変更あり かつ 保存先ありなら保存」という小さなメソッドで、`NoteNestFormatRoundTripTests`（v2.16.13 TD-63 で `NoteNestFormatSchemaRegressionTests` から分割）から直接テストされている。名前は AutoSave だが Shell の自動保存機構とは独立した保存ヘルパー | 参照テストと将来の呼び出し元がなくなったとき |
...
- 現行自動保存のテストは `AutoSaveCandidatePolicyTests`（Shell 側ポリシー）と
  `NoteNestFormatRoundTripTests`（`ProjectLifecycleService.TryAutoSave` を直接検証。
  v2.16.13 TD-63 で `NoteNestFormatSchemaRegressionTests` から分割）に存在する。
