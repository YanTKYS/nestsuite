...
> **[履歴文書]** この文書は v1.7〜v1.8 時点の統合設計メモです。記載された AppShell / Workspace 統合は実装済みであり、現行コードの参照元ではありません。詳細は [`docs/integration/README.md`](../../integration/README.md) を参照してください。
...
| ~~v1.8.2~~ | ~~IdeaNest `.ideanest` 保存・読込方針の整理（N21 準備）~~ → v1.8.2 で完了（モデルに `[JsonPropertyName]` 追加・`IdeaNestFileService` スケルトン・`docs/archive/completed-designs/ideanest-save-load-plan.md` 作成） |
