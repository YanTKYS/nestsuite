...
> **[履歴文書]** この文書は v1.9.4 時点の設計メモです。記載された NoteNest 複数ファイルタブ対応は v1.9.5 以降で実装済みであり、現行コードの参照元ではありません。詳細は [`docs/integration/README.md`](../../integration/README.md) を参照してください。
...
MainViewModel
  ...
  └─（DispatcherTimer × 2）    ← AutoSaveTimer・UnsavedTimer（AutoSaveTimer は v2.14.13 TD-61 で撤去済み。現行の自動保存は NestSuiteShellWindow.AutoSave.cs が担う）
```
...
| AutoSave タイマー | ~~`MainViewModel._autoSaveTimer`~~（v2.14.13 TD-61 で撤去済み。現行の自動保存は `NestSuiteShellWindow.AutoSave.cs`（SH-33）が Shell 側で一元的に担う） | タブごとに独立した間隔（当時の設計案。現行は不採用） |
...
**デメリット：**
- `DispatcherTimer` が各インスタンスに 2 本（AutoSave + UnsavedTimer）。10 タブ開くと 20 本になる（当時の懸念点。AutoSave タイマーは v2.14.13 TD-61 で `MainViewModel` から撤去済みのため、現行はこの懸念自体が解消済み）
