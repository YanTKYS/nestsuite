...
> **位置づけ（v2.16.29 TD-75c で整理）:**
> この文書は、TD-28（v2.10.14）/ TD-30（v2.10.16）/ TD-31（v2.10.17）/ TD-32（v2.10.18）時点の
> テストクラス分類・棚卸し結果と、その対応記録を残した**過去分析スナップショット**である。
> 分類表の件数・クラス名は当時のものであり、現状とは一致しない。この文書は削除せず、
> 過去の分類・整理の判断経緯を確認するための参考資料として残す。
>
...
> 「新規テスト追加先判定ガイド」は、そのルールを適用する際の受け皿一覧・参考資料として
> 引き続き利用できる（規範は guidelines 側にある）。
