...
この差異は `FileSaveStateSync.cs` に集約した。VM レベルの意味は `ChatNestMessageEditingTests.MarkSaved_WhenInputTextRemains_HasUnsavedChangesIsTrue`
（v2.16.13 TD-63 で `ChatNestWorkspaceViewModelTests` から分割）が固定している。
...
4. 構造は `NestSuiteShellSaveFlowTests`（v2.16.13 TD-63 で `NestSuiteShellWorkspaceLaunchTests` から分割）の TD-45 セクションのリフレクションテストで固定されている
