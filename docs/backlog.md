...
| No | 方針 | 概要 | 見送り理由 |
|----|------|------|------------|
| RJ-1 | .NET Framework 4.8 版の正式採用見送り | 実機起動は確認できたが複数 DLL 構成となるため正式採用しない。検証用プロジェクトも保持しない | 単一 EXE 配布方針（RJ-5）と衝突。Costura.Fody / ILRepack 等の単一 EXE 化手段が確立された場合のみ再検討 |
