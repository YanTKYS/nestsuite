...
| LK-3 | TempNest スロット → IdeaNest カード追加 | TempNest スロットのタイトル・本文を IdeaNest タブのカードとして追加する。LK-2 の IdeaNest 版（旧 TN-7 を吸収）。TD-92 設計で適用可能性を確認済み（LK-4 で追加した IdeaNest 側受入 API `AddCardFromTransfer` をそのまま再利用できる）。**着手トリガー・実装主体: LK-2 と同じ** | C |
