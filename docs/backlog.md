...
SH-1 は v2.16.11、SH-15 は v2.16.3、SH-19 は v2.16.4、SH-28 は v2.16.5、SH-29 は v2.16.9、SH-30 は v2.16.10 で実装済み（欠番）。詳細は `docs/release-notes.md` 参照。

| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| SH-24 | タブのクイックスイッチャー強化 | タブ過多時のキーボード検索・切替補助。既存の `Ctrl+Tab` やオーバーフロー一覧（SH-6）との関係を整理してから検討する | C |
| SH-34 | 復元失敗通知と FileNotFound 再試行解除確認の 1 ダイアログ統合 | TD-70 では復元失敗通知の後に FileNotFound 解除確認を別ダイアログで表示している。ネットワークドライブ未接続などで毎回「いいえ」を選ぶ利用者は、起動時に 2 枚のダイアログを閉じることになる。将来、復元失敗通知内に解除選択を統合し、1 ダイアログ化できるか検討する | C |
| SH-35 | InvalidFormat 等の恒久 pending entry への案内・解除拡張検討 | InvalidFormat / AccessDenied / SchemaVersionTooNew は TD-70 の解除対象外であり、恒久的に解消されない場合は毎起動で復元失敗通知が出続ける。当面は、ファイルを修復する・単体で開いて `.bak` 復元案内を見る・不要なら削除して FileNotFound 化して解除する、という間接経路で許容する。将来、FAQ 記載または解除対象拡張を検討する | C |
