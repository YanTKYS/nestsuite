...
| TD-75 | 静的テスト棚卸しレビューのフォローアップ | `docs/planning/static-test-inventory-review.md`（TD-74、v2.16.25）で整理した次工程の実装。**TD-75a（v2.16.26）で完了**: `NestSuiteDocsContractTests.cs` 内の release notes / backlog 存在確認テスト（バージョン・ID 存在確認、backlog open item 不在確認、backlog 完了済み範囲確認の 3 パターン、同形 Fact 53 件）を (version, id) ペアの `[Theory]` + `MemberData` + 共通 helper に集約し、失敗時にどの version / id か分かるメッセージにした。**TD-75a-2（v2.16.27）で完了**: 他のテストファイル（SchemaVersioningPolicyTests / TempNestTests / ChatNestWorkspaceFeatureRecordsTests / SaveAllCommandTests / MarkdownExportTests / AtomicFileWriterTests / SessionTabMapperTests / SessionNestGuardNestPolicyTests / PromptStandardContractTests / ChatNestExportFormatterTests / ExpertProposalPlanningTests）に散在していた release notes / backlog の機械的な存在確認テスト 35 件を `NestSuiteDocsContractTests.cs` へ移設した（既存データ群へのデータ行追加、または同等の独立 Fact への移設。重複していたチェックは統合し、TD-24 / TD-25 のバージョン取り違え・SH-19 の確認方向の誤りは移設にあわせて修正した）。検証内容・網羅対象は変えていない。**TD-75b（v2.16.28）で完了**: 挙動テスト化候補 2 件を置換した。`SessionTabMapper_IsSessionPersistable_StillExcludesTempTabs`（private 実装のソース文字列確認）は `SessionTabMapper.CreateSessionState` の出力確認（Temp タブが除外され通常タブは残ることを確認）へ置換した。`Constructor_SavesSessionWhenForgotFileNotFoundDuringStartup_EvenIfRestoreReturnedFalse`（コンストラクター内の条件式・文順のソース文字列確認）は、新設した UI 非依存の `StartupRestoreSessionPolicy.ShouldSaveSessionAfterStartupRestore` policy helper の単体テストへ置換し、コンストラクターはその helper を呼ぶよう最小修正した。**TD-75c（v2.16.29）で完了**: `test-classification-analysis.md` を「TD-28 / TD-30 / TD-32 時点の過去分析スナップショット」として位置づけを明確化した。現在の静的テスト判断基準は `static-test-guidelines.md`（TD-73）と `static-test-inventory-review.md`（TD-74）を優先し、同文書は過去の判断経緯を確認する参考資料として残す（§12 の新規テスト追加先判定ガイドは受け皿一覧の参考として利用可、規範は guidelines 側）。**TD-75d（v2.16.30）で完了**: 削除候補 13 件の削除判断レビューを実施し、`docs/planning/static-test-deletion-candidate-review.md` として文書化した。判断結果は Delete candidate 10 件（凍結 planning 文書の章構成固定 5 件 + 古い UI 削除ガード 5 件）/ Keep 3 件（文書存在確認・バグ履歴と直結するフォント種類ガード・「新 Workspace を作らない」設計境界）/ Replace then delete 0 件 / Defer 0 件。テストの削除・skip は行っていない。**残作業**: 同レビューの Delete candidate 10 件の実削除をユーザー承認の上で行う（削除時は同文書の一覧を PR に転記し、Keep 3 件を誤って削除しないこと。`ExpertProposalPlanningTests.cs` 自体は削除しない）。既存テストの保証内容を弱めないこと | C |
