...
SH-1 は v2.16.11、SH-15 は v2.16.3、SH-19 は v2.16.4、SH-28 は v2.16.5、SH-29 は v2.16.9、SH-30 は v2.16.10、SH-34 は v2.16.21、SH-37 は v2.18.3、SH-38 は v2.18.5、SH-39 は v2.18.15、SH-40（AT-1 フェーズ1）は v2.18.17、SH-41（AT-2 フェーズ1）は v2.18.18、SH-42（魅力向上施策の実機回帰・総点検）は v2.18.19 で実装済み（欠番）。SH-24（タブのクイックスイッチャー強化）は v2.18.23 の総点検（TD-89）で見送り（欠番。既存の Ctrl+Tab / Ctrl+1〜9 / タブ一覧オーバーフローで到達でき、タブ過多で切替に迷う実利用報告もないため。再提案時は新 ID を採番する）。詳細は `docs/release-notes.md` と `docs/planning/backlog-adoption-trigger-review.md` 参照。
...
| M15 | マーカー / タスクの一括コピー | 右ペインのマーカー一覧またはタスク一覧に「全件コピー」ボタンを追加し Markdown リスト形式でクリップボードへコピーする。保存スキーマ変更なし。既存エクスポート系（ID-10 / ChatNest エクスポート）と語彙を揃える。**実装主体: 通常エンジニアで実装可能（着手トリガー不要）** | B |
...
ID-15 は v2.18.2 で実装済み（欠番）。ID-14 は v2.18.8 で実装済み（欠番）。ID-6 は v2.18.14 で実装済み（欠番）。ID-10 は v2.18.16 で実装済み（欠番、Markdown 出力のみ。CSV は今回未実装）。ID-5（複数選択・一括操作）・ID-13（簡易統計）は v2.18.23 の総点検（TD-89）で見送り（欠番。ID-5 は単体新機能凍結方針と衝突し複数選択モデルの導入コストが大きく実利用要望なし。ID-13 は ID-14 の色チップ件数表示で実質充足し常設情報の追加は認知負荷軽減方針と逆行するため。再提案時は新 ID を採番する）。詳細は `docs/planning/backlog-adoption-trigger-review.md` 参照。

| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| ID-4 | カード一覧のキーボード操作 | Enter でフォーカス中カードのプレビューを開く（必須。現状は Tab 到達・フッターボタン・ContextMenu で操作可能だが Enter が効かない）。矢印キーでのカード間移動・Space ピン留めは任意（Tab・既存ボタンで代替可のため実機評価後に判断）。Delete キー即削除は採用しない。範囲の根拠は `docs/planning/keyboard-accessibility-cross-review.md` §8（TD-88、設計レビュー済み）。**実装主体: 通常エンジニアで実装可能（必須部分は範囲固定済み・着手トリガー不要）** | A |
...
TD-1〜TD-63、TD-64〜TD-76 は完了済み（欠番）。TD-77 は v2.17.9、TD-78 は v2.17.3、TD-79 は v2.17.4、TD-80 は v2.17.5、TD-81 は v2.17.6、TD-82 は v2.17.7、TD-82-1 は v2.17.8、TD-84 は v2.18.13、TD-86 は v2.18.20、TD-88 は v2.18.22、TD-89 は v2.18.23、TD-90 は v2.18.24 で完了済み（欠番）。詳細は `docs/release-notes.md` 参照。
...
| TD-87 | recent files 破損時の quarantine 退避 + ErrorLog 記録 | `NestSuiteRecentFilesService.Load` は破損 JSON を黙殺して空履歴を返しており、session（`.corrupt` 退避+ErrorLog、TD-65）と復旧方針が非対称。session/TD-65 と同型の quarantine 退避＋ErrorLog 記録を最小実装する。実装範囲は `docs/planning/state-data-protection-boundary-review.md` §6 L1・§9 で確定済み（設計レビュー済み・1 version 規模）。recent files は再蓄積可能な補助状態のため急がない。TD-90（`docs/planning/storage-compatibility-migration-distribution-review.md` §9）で設計範囲の妥当性を再確認済み。着手時は tempnest.json の同型対応（同書 ST-4a）を同時に判断してよい。**実装主体: 通常エンジニアで実装可能（着手トリガー不要・範囲固定済み）** | C |
