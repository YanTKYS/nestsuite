...
TD-1〜TD-63、TD-64〜TD-76 は完了済み（欠番）。TD-78 は v2.17.3、TD-79 は v2.17.4、TD-80 は v2.17.5、TD-81 は v2.17.6 で完了済み（欠番）。詳細は `docs/release-notes.md` 参照。
...
| TD-82 | 旧NoteNest・完了済み設計文書のarchive再分類 | `docs/planning/docs-inventory-and-archive-policy.md` の次回候補に基づき、旧NoteNest Classic、完了済み editor / extension 設計、旧入口 README の扱いを再分類する。移設・削除は小さな文書群単位で判断し、Delete Candidate の即削除は行わない | C |
