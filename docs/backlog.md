...
L8・L20 は v2.16.8 で実装済み（欠番）。L23 は v2.18.1 で実装済み（欠番）。L25 は v2.18.5 で実装済み（欠番）。M19 は v2.18.6 で実装済み（欠番）。L24 は v2.18.7 で実装済み（欠番）。M14 は v2.18.9 で実装済み（欠番）。詳細は `docs/release-notes.md` 参照。
...
| M18 | NoteNest 画面 XAML のペイン別 UserControl 分割 | 大規模な NoteNestWorkspaceView.xaml を左ペイン、エディタ、タスク、マーカー単位の UserControl に分割し、既存 ViewModel と操作契約は維持する。画面修正時に追う範囲を限定して認知負荷と技術的負債を減らす | B |
...
TD-1〜TD-63、TD-64〜TD-76 は完了済み（欠番）。TD-77 は v2.17.9、TD-78 は v2.17.3、TD-79 は v2.17.4、TD-80 は v2.17.5、TD-81 は v2.17.6、TD-82 は v2.17.7、TD-82-1 は v2.17.8、TD-84 は v2.18.13、TD-86 は v2.18.20 で完了済み（欠番）。詳細は `docs/release-notes.md` 参照。
