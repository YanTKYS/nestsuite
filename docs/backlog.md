...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| LK-2 | TempNest スロット → NoteNest 昇格 | TempNest スロットの本文を既存 NoteNest タブの新規ノートとして転送する。旧 TN-7（ターゲットタブ選択・転送後クリア確認・明示操作のみ）を吸収。TD-92 設計で適用可能性を確認済み（NoteNest 側にタイトルを受け取るオーバーロードの追加が LK-2 の作業に含まれる）。**着手トリガー・実装主体: LK-4 の実装完了後、実際に共通性が確認できた部分だけを再利用して通常エンジニア実装（先行抽象化はしない）** | C |
| LK-3 | TempNest スロット → IdeaNest カード追加 | TempNest スロットのタイトル・本文を IdeaNest タブのカードとして追加する。LK-2 の IdeaNest 版（旧 TN-7 を吸収）。TD-92 設計で適用可能性を確認済み（LK-4 で追加する IdeaNest 側受入 API をそのまま再利用できる）。**着手トリガー・実装主体: LK-2 と同じ** | C |
| LK-4 | ChatNest 発言 → IdeaNest カード化 | ChatNest の個別発言から既存 IdeaNest タブへカードとして直接追加する。思考の流れ（断片→発想→対話→記録）上で最も自然なため**着手時の第一候補**。**着手条件は v2.20.1 / TD-92 の転送共通ヘルパー設計レビュー完了により成立済み（未実装・次の実装候補）。実装主体: 通常エンジニア実装（設計正本 `docs/planning/workspace-manual-transfer-helper-design.md` に従う。責務境界・DTO・転送先識別・受入方法・結果契約・dirty/save/エラー責務・Title/Body マッピング・IdeaNest タブ 0/1/複数件時の挙動・変更予定ファイル・必須テストは確定済み）** | C |
