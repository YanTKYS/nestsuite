...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-59 | `.nestsuite` オープン時の二重読込・二重パース解消 | `.nestsuite` を開く全経路（Open ダイアログ・起動引数・最近ファイル・セッション復元・pipe・ファイル関連付け）で、種別判定と本読込が同じファイルを複数回読み込み解析している。**TD-59a（v2.16.32）で設計レビュー完了**: `docs/planning/nestsuite-double-read-design-review.md` に全経路の棚卸し（主要経路は `FromFilePath` の再判定を含め実際は 3 回読込だったことを特定）、設計案比較、採用案（1 open operation 限定の `WorkspaceFileOpenContext` を明示的に引き渡し、FileService に `Load(path, preloadedEnvelope)` overload を追加。process-wide / path-based cache は不採用）、API スケッチ、TOCTOU（1 operation = 1 スナップショット）、kind / schema / failure 責務、テスト戦略を確定した。**残作業（TD-59b、設計文書 §13 の分割に従う）**: TD-59b-1 基盤 API（`ReadFromFile` / `TryPrepareOpen` / `FromResolvedKind`）→ TD-59b-2 FileService overload → TD-59b-3 Shell 経路切替 → TD-59b-4 session 復元経路 → TD-59b-5（任意）保存後 probe の非読込化と全体回帰。注意点: `TryGetKind` の単一集約点という FM-1 の設計利点を壊さない・kind 不一致検証（`EnsureKind`）を削除しない・レガシー拡張子読込を壊さない | C |
