...
SH-1 は v2.16.11、SH-15 は v2.16.3、SH-19 は v2.16.4、SH-28 は v2.16.5、SH-29 は v2.16.9、SH-30 は v2.16.10、SH-34 は v2.16.21、SH-37 は v2.18.3 で実装済み（欠番）。詳細は `docs/release-notes.md` 参照。
...
| SH-38 | TN-3 昇格直後のタブ選択表示の不整合修正 | `PromoteTempNestSlotToNoteNest` の `ActivateTab(tab)` が、`CreateNoteFromTransfer` 中の `ReplaceTab`（IsModified 同期）で置き換え済みの古いタブ record を渡すため、タブストリップの選択ハイライトが新規タブへ移らない。TabDetach と同じ Id 再解決（`_tabs.First(t => t.Id == tab.Id)`）へ修正する。review7-fable5 REV7-1（Medium）。L25 と同一バージョンでの対応を推奨 | A |
...
L8・L20 は v2.16.8 で実装済み（欠番）。L23 は v2.18.1 で実装済み（欠番）。詳細は `docs/release-notes.md` 参照。
...
| L24 | タスクの関連ノート選択を一覧化 | 現在のノート名完全入力を NotePickerDialog に置き換え、ノートブック名付き一覧から選択できるようにする。表記を覚えて入力する負荷とタイプミスをなくし、一覧の絞り込み実装も共用する。タスクは縮退方針（TD-52）の互換機能のため優先度を A から B へ調整（review7-fable5 §5） | B |
| L25 | project 再読込後の空状態ガイド更新漏れ修正 | `NoteWorkspaceViewModel.Load` が suppress 中のコレクション変更後に `Changed` を発火しないため、既存 NoteNest タブで別ファイルを開いた直後に L23 の空状態ガイド（および従来の `IsNoteListEmpty` / `HasAnyNotes`）が古い表示のまま残る。Load 完了時に 1 回 `Changed` を発火する等で通知し、再読込後の表示条件更新をテストへ追加する。review7-fable5 REV7-2（Medium）。SH-38 と同一バージョンでの対応を推奨 | A |
...
| M19 | UI 設定・最近使ったファイルの復旧経路 | UiSettingsService と RecentFilesService の読込／保存失敗を握り潰すだけにせず、破損ファイルの退避、既定値への復旧、非モーダルな通知を共通化する。設定消失時の原因不明状態を避け、保守性と UI/UX を改善する | B |
...
ID-15 は v2.18.2 で実装済み（欠番）。詳細は `docs/release-notes.md` 参照。
...
| CH-18 | ChatNest 会話一覧の視線誘導改善 | 発言者色・アクセント・配置で最低限の識別はできているが、長い会話では発言者の切り替わりや未送信入力との境界を追いにくい。日付区切り（CH-11）や操作発見性整理（CH-16）と競合しない範囲で、発言者ラベルの視認性、入力欄との余白、選択中メッセージの軽い強調を見直し、UI/UX を改善する | B |
