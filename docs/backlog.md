...
| L23 | 空状態での次操作ガイド | ノートブック／ノート／タスク／マーカーが空の領域に、追加・選択など次に取れる操作を短く表示する。空白の意味を推測する認知負荷を減らし、初回利用時の UI/UX を改善する | A |
| L24 | タスクの関連ノート選択を一覧化 | 現在のノート名完全入力を NotePickerDialog に置き換え、ノートブック名付き一覧から選択できるようにする。表記を覚えて入力する負荷とタイプミスをなくし、一覧の絞り込み実装も共用する | A |
