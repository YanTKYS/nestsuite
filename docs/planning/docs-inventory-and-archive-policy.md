...
| `docs/development/workspace-file-extension-unification.md` | Active Reference | `.nestsuite` wrapper・保存・読込・legacy互換方針の補助正本 | productionコード・schema policy・backlog FM-1・release notes | 完了済み移行履歴だけでなく、現行 wrapper / 保存 / 読込 / 互換仕様を保持している | TD-82-1 で development 配下へ復帰。将来 Canonical なファイル形式文書へ仕様分離できた場合のみ再archiveを検討。 |
...
- 補助参照: `docs/development/workspace-file-extension-unification.md`
...
### TD-82完了 / TD-82-1補正: 旧NoteNest・完了済み設計文書のarchive再分類

TD-82 では 2 文書を再確認したが、v2.17.8 / TD-82-1 で `workspace-file-extension-unification.md` の正本性を補正した。

- `docs/archive/legacy-notenest/classic-code-contraction.md`: Classic 残存コード縮退記録は完了済み履歴であり、現行 NestSuite / NoteNest Workspace の正本ではないため Archived のまま維持する。
- `docs/development/workspace-file-extension-unification.md`: `.nestsuite` 拡張子統一の完了履歴だけでなく、現行 wrapper・保存・読込・legacy 互換仕様を保持し、production コードおよび Canonical 文書から継続参照されるため Active Reference へ戻した。

補正後の扱い:

- `workspace-file-extension-unification.md` は現行仕様の補助正本として development 配下に置く。
- 将来、Canonical なファイル形式文書へ wrapper・保存・読込・legacy 互換仕様を分離できた場合にのみ、同文書の再archiveを検討する。
- archive 文書だけを根拠に Classic 経路復活、旧機能復活、旧拡張子互換削除、保存形式変更を行わない。
...
### v2.17.9: 完了済みeditor設計・旧入口READMEのarchive/削除候補再判定
