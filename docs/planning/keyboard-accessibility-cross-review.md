...
**K-2 は v2.19.10 / L26（マーカー一覧）・v2.19.11 / L27（アウトバウンドリンク一覧・バックリンク一覧）で全部分対応済み。** いずれもItemsControlからListBox化し、上下キー選択・Enterジャンプに対応した。ジャンプ処理はマウスクリックと共通の処理（マーカー: `MarkerClickCommand`、リンク: `TryNavigateOutboundLink`／`TryNavigateBacklink`）をそのまま使用している。詳細は `docs/release-notes.md`（v2.19.10 L26・v2.19.11 L27）を参照。K-3〜K-6は本レビュー時点のまま未対応（今回変更していない）。
