...
**K-1 は v2.19.9 / SH-44 で対応済み。** パネル内Escapeで閉じ、×ボタンと共通のクローズ処理を使用し、通常クローズ後は開く前のフォーカス（無効な場合は安全なフォールバック）へ復帰する。検索結果を実行して遷移した場合は遷移先フォーカスを維持する。詳細は `docs/release-notes.md`（v2.19.9 SH-44）を参照。K-2〜K-6は本レビュー時点のまま未対応（今回変更していない）。
