...
**K-1 は v2.19.9 / SH-44 で対応済み。** パネル内Escapeで閉じ、×ボタンと共通のクローズ処理を使用し、通常クローズ後は開く前のフォーカス（無効な場合は安全なフォールバック）へ復帰する。検索結果を実行して遷移した場合は遷移先フォーカスを維持する。詳細は `docs/release-notes.md`（v2.19.9 SH-44）を参照。

**K-2 は v2.19.10 / L26（マーカー一覧）・v2.19.11 / L27（アウトバウンドリンク一覧・バックリンク一覧）で全部分対応済み。** いずれもItemsControlからListBox化し、上下キー選択・Enterジャンプに対応した。ジャンプ処理はマウスクリックと共通の処理（マーカー: `MarkerClickCommand`、リンク: `TryNavigateOutboundLink`／`TryNavigateBacklink`）をそのまま使用している。詳細は `docs/release-notes.md`（v2.19.10 L26・v2.19.11 L27）を参照。
...
**K-5 は v2.19.14 / SH-45 で対応済み。** アイコン・記号のみで操作目的が可視文字から伝わらないボタン（タブ×・横断検索×・タブ追加＋・タブ一覧▾・NoteNestコメント編集✏/保存💾/右ペイン開閉»«、IdeaNestタグパネル切替・検索クリア✕・再シャッフル🔀・ランダムプレビュー🎲・カードサイズS/M/L・ピン留め📌/アーカイブ📥/削除🗑、ChatNest検索閉じる✕/前▲/次▼）へ`AutomationProperties.Name`を付与した。可視テキストからUIA名を導出できるコントロール（通常ボタン・Header付きMenuItem）には機械的に付与していない。状態によって操作が逆転するボタン（IdeaNestのピン留め・アーカイブ切替、NoteNest右ペイン開閉）は状態に追従する名前にした。詳細は `docs/release-notes.md`（v2.19.14 SH-45）を参照。

**K-6 は v2.19.15 / SH-46 で対応済み。** ファイルメニューの「新規作成(_N)」／「名前を付けて保存(_N)」重複を、「名前を付けて保存」を`_V`へ変更して解消した。タブContextMenuの「このタブへ戻す(_R)」／「右側のタブを閉じる(_R)」重複を、「このタブへ戻す」を`_B`へ変更して解消した。表示文言・メニュー順序・Clickハンドラ・Command・InputGestureText・保存処理・タブ操作ロジック・SH-45のAutomationPropertiesは変更していない。詳細は `docs/release-notes.md`（v2.19.15 SH-46）を参照。

**これにより、本レビュー（TD-88）のK-1〜K-6はすべて完了した。** K-1（v2.19.9 SH-44）・K-2（v2.19.10 L26・v2.19.11 L27）・K-3（v2.19.12 L28）・K-4（v2.19.13 CH-19）・K-5（v2.19.14 SH-45）・K-6（v2.19.15 SH-46）。ただし、各versionともWindows実機でしか確認できない項目は個別のrelease notesに記載のとおり未確認のまま残っている（§10「未確認事項」も参照。実機確認自体は本レビューの対象外）。

| ID | 重要度 | 対象 | 再現手順 | 利用者影響 | 現行の代替経路 | 推奨対応 | 今回実装するか |
|----|--------|------|----------|-----------|----------------|----------|----------------|
| K-1 | Medium | Shell横断検索パネル（`NestSuiteShellWindow.CrossSearch.cs`） | Ctrl+Shift+Fで開く→Escapeを押す→閉じない。×ボタンで閉じると直前のフォーカス位置に戻らない | Escapeで閉じる他画面との一貫性を欠き、閉じた後に作業位置へ戻るTab操作が余分に必要 | Ctrl+Shift+F再押下で閉じられるため完結性は保たれている（AT-2レビュー§17でも独立改善として記録済み） | パネル内Escapeで閉じ、閉じたら直前のフォーカス（またはアクティブWorkspace）へ戻す小修正。1 version | **対応済み（v2.19.9 / SH-44）** |
...
| K-6 | Low | Shellファイルメニューのアクセスキー重複 `新規作成(_N)`/`名前を付けて保存(_N)`（27-68行）、タブContextMenuの `_R` 重複（456/461行） | Alt+F→Nで確定せず循環選択になる | Enter1打が余分に必要なだけで操作は可能（WPF標準の循環動作） | 矢印キー選択 | どちらかのアクセスキー変更 | **対応済み（v2.19.15 / SH-46）** |
...
**鮮度補正（v2.19.11時点）**: 上表の「Enterでフォーカス中カードのプレビューを開く」（必須部分）は v2.19.1 / ID-4 で実装済み（`docs/release-notes.md` 参照）。矢印キーでのカード間移動・Spaceでのピン留め切替・Deleteキーでの削除は、上表の判定（任意／採用しない）のとおり今回まで未実装のまま。IdeaNestのコードは本version（L27）では変更していない。
...
| 順 | 内容 | 対応する指摘 | 規模 |
|----|------|--------------|------|
| 1 | ~~Shell横断検索: Escapeで閉じる＋閉じた後のフォーカス戻り~~ **完了（v2.19.9 / SH-44）** | K-1 | 1 version（Shellのみ） |
| 2 | ~~IdeaNest: Enterでフォーカス中カードをプレビュー（ID-4必須部分）~~ **完了（v2.19.1 / ID-4）** | §8 | 1 version（IdeaNestのみ） |
| 3 | ~~NoteNest: マーカー一覧のListBox化（選択+Enterでジャンプ）~~ **完了（v2.19.10 / L26）** | K-2の一部 | 1 version |
...
| 8 | ~~アクセスキー重複の解消~~ **完了（v2.19.15 / SH-46）** | K-6 | 1 version |
