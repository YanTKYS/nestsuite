...
**K-2 マーカー一覧部分は v2.19.10 / L26 で対応済み。** マーカー一覧をItemsControlからListBox化し、上下キー選択・Enterジャンプに対応した。ジャンプ処理はマウスクリックと共通の`MarkerClickCommand`をそのまま使用している。K-2のうちアウトバウンドリンク一覧・バックリンク一覧は今回のスコープ外で、本レビュー時点のまま未対応。詳細は `docs/release-notes.md`（v2.19.10 L26）を参照。K-3〜K-6は本レビュー時点のまま未対応（今回変更していない）。
...
| K-2 | Medium | NoteNest右ペイン マーカー/アウトバウンドリンク/バックリンク項目（`NoteNestWorkspaceView.xaml` 776/899/958行） | Tabを何度押しても一覧項目にフォーカスが来ない。ジャンプは`MouseLeftButtonDown`のみ | マーカー・リンク経由の該当行ジャンプがマウス必須 | Ctrl+F検索で同じ文字列に到達可能。リンク先ノートはツリーから開ける | ItemsControl→ListBox化（選択+Enterでジャンプ）を一覧単位で行う小修正。1 version 1一覧 | **マーカー一覧部分は対応済み（v2.19.10 / L26）。リンク一覧部分はしない（別version候補）** |
