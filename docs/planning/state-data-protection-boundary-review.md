...
|---|--------|------|------|------|------|
| L1 | Low | `NestSuiteRecentFilesService.Load` | 破損JSONを`catch { return []; }`で黙殺し、履歴全消失が無通知・無退避・ErrorLogなし。session（`.corrupt`退避+ErrorLog、TD-65）・UI settings（M19復旧通知）と復旧方針が非対称 | recent files JSONが破損した場合のみ。履歴は利用で再蓄積可能な補助状態のため実害は小 | **文書化のみ（本記録）**。将来、session同様のquarantine+ErrorLogを加える小修正候補（TD-87候補、1 version規模）。正本でないため急がない |
