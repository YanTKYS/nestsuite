...
- **`docs/` の Markdown を 79 → 13 ファイル、総行数 28515 → 9521 行にした（release notes 6464 行を除くと 22051 → 3057 行）。** ディレクトリは `archive/` `design/` `integration/` `migration/` `planning/` が消滅し、`architecture/` 3 → 1、`development/` 17 → 6、`testing/` 4 → 1 になった。**削除ファイルの一覧は Git 履歴が正本であり、ここへは転記しない。**
...
- **統合（第 2 段階）**: `nestsuite-designer-guidelines.md` を `nestsuite-development-guidelines.md` へ統合し、設計者側の内容（課題選定・プロンプト標準契約・通常エンジニア/エキスパートの使い分け・引継ぎ・レビュー確認）は §13〜§16 として残した。**実装者と設計者でガイドラインを分けるのをやめた**（重複した「本指示 > guideline」「保存形式を変えない」等の規則が両方に書かれていたため）。**静的テスト方針の二重正本を解消**し、`static-test-guidelines.md` を削除して、WPF `Window` をインスタンス化できないための例外的なソース走査の扱いを `test-suite-policy.md` §5.1 へ集約した（正本は `test-suite-policy.md`）。NoteNest エディタ関連の 3 文書（host / adapter / textbox-dependencies）は、現行の判断（WPF 標準 TextBox 方針の維持）が開発ガイドライン §4 と backlog RJ-8 / RJ-11 にある重複であったため削除した。Workspace の責務・XAML 構造の索引文書 4 本は、境界ルールを開発ガイドライン §12 へ吸収して削除した。`nestsuite-known-limitations.md` は利用ガイドの「既知の制約」へ、schema 変更の判断表と必須テスト一覧は `schema-versioning-policy.md` へ吸収した。開発ガイドラインは 2 文書合計 864 行 → 315 行。
- **簡素化（第 3 段階）**: 残した文書から、作成 version・backlog ID・過去の変更履歴・「以前は○○だった」「vX.X で○○した」の記述、繰り返しの規則、長い背景説明を削除した。リリース前チェックリストは見出しから `（v2.19.0 SH-43）` 形式の版・ID ラベルを外し（27 箇所）、確認項目そのものは減らしていない。`docs/README.md` は 29 行の入口（まず読むもの 4 件 / 作業に応じて読むもの 9 件）にして、docs 一覧・分類台帳としては使わないことを明記した。ルート `README.md` の文書表も、開発向けは `docs/README.md` へ委譲する形へ縮めた。
...
- **`docs/planning/workspace-manual-transfer-helper-design.md`（809 行）は削除し、Workspace 間手動転送の契約正本を production 側（`NestSuiteShellWindow.WorkspaceTransfer.cs`）へ移した。** 当初はここだけ節番号を維持して残す判断をしていたが、(1) 文書の `InvalidContent` 定義が「Body が空」のままで、Title / Body のどちらか一方があれば有効という**現行実装およびテストと矛盾していた**、(2) production コメントが `設計正本: …md §14` と文書を指し、文書側が「production から節番号で参照されるため節番号を変えない」と書く**循環参照**になっていた、という 2 点から削除に切り替えた。責務境界・5 つの結果値の意味・0/1/複数件と Detached の扱い・dirty / save 契約・例外処理の範囲は、共通ヘルパーの doc コメントへ現行実装として書き直してある。回帰は `LK2` / `LK3` / `LK4` / `WorkspaceTransferContractTests` が behavior レベルで確認している。
- **`docs/backlog.md`** は未着手・保留・トリガー待ち・見送り方針のみに戻した。完了済み項目の経緯説明（K-1〜K-6 の対応履歴、TD-93 の総点検内容、TD-59 の実装経緯、LK / TN / CH / FM の個別完了記述など）を削除し、**再利用しないための欠番一覧**と、現在も生きている着手トリガー・見送り理由だけを残した。各項目内の `（v2.13.7 TD-54 実施済み）` のような版・ID ラベルも外している。未完了 24 項目の実体は変更していない。**「念のため」の新規 backlog 項目は追加していない。**
- **新しい文書は 1 つも追加していない。** 棚卸し報告・削除一覧・新 docs 方針といった文書は作っていない（それ自体が次の棚卸し対象になるため）。
- **残した文書を現行 production と突き合わせて修正した。** 利用ガイドの「サイドバーのツールボタンからもタブを作成できる」は現行 UI にない導線（各 Nest の新規作成は ファイル > 新規作成 とタブバーの「＋」に集約済み）だったため書き換え、リリース前チェックリストからは成立しないツールメニュー確認 4 項目を現在のツールメニュー（横断検索・移行パック入出力）の確認へ差し替えた。未保存マークの表記 `*` → `●`（現行 XAML は `Text="●"`）、別ウィンドウ再統合メニューのアクセスキー `(_R)` → `(_B)` も実装に合わせた。可視文字列・アクセスキー・ショートカットは機械的に全件突合し、残る不一致は 0 件。
- **リンク切れ 0 件。** `docs/` 配下・ルート `README.md` の相対 Markdown リンクはすべて解決する（`DocsLinkIntegrityTests`）。加えて、Markdown リンクではない本文中の文書名参照も走査し、production コメント 5 ファイル・テストコメント 21 ファイル・`reference/legacy/README.md`・残した docs 3 ファイルから、削除した文書への参照を除去した（version / backlog ID の記述自体は残している）。**docs 本文を検証する新しい assert は追加していない。**
...
- **コメントを現行化した（履歴マーカーを含むコメント行 約 844 行を対象）。** `// v2.16.5 SH-28:` `// TD-59b-3 対応` `// LK-4 (v2.21.0):` のような version / backlog ID / review 由来のラベル接頭辞を除去し、本文の WHY だけを残した。`// 従来は … だった` `// v1.9.2 以降で … へ置き換える` のような変更履歴の語りは現在形の WHY へ書き換え、役割を終えた将来予定・段階説明（`<para><b>v1.7.3 の位置づけ</b>` 等の changelog 段落）は削除した。コメント行は 3681 → 3608 行だが、削減より **内容の現行化**が主眼である。
- **XAML コメントも同じ基準で現行化した（コメントブロック約 126 件）。** `.xaml` も production source であるため、`<!-- v2.16.10 SH-30: … -->` `<!-- CH-5: 検索バー -->` `<!-- LK-4 (v2.21.0): … -->` 等のラベル接頭辞を除去し、`<!-- ItemsControl → ListBox 化。… -->` `<!-- 閉じるボタンが中央寄りに見える不具合を修正。旧実装は… -->` のような移行・修正の経緯を、現在その書き方を選んでいる理由（WPF の `LastChildFill` が `Dock="Right"` を無視する等）へ書き換えた。`<!-- Header -->` `<!-- Tag chips -->` のような構造の見出しラベルは、長い XAML の位置把握に役立ち履歴でもないため維持している。
...
- **利用者向け挙動・保存形式・互換性契約の変更はない。** ただし「public API 無変更」ではなく、未使用の C# `public` メンバー（`IExporter` interface、`IdeaConfirmWindow.ShowYesNoCancel`、`ConfirmResult.Secondary`、`ChatNestWorkspaceViewModel.Speakers` 等）は dead code として削除している。NestSuite は外部へライブラリ API を提供するプロジェクトではないため、内部的に public だった到達不能コードの削除は本 version の目的に含まれる。
...
- **production 規模**: `.cs` はファイル 243 → 242、総行数 23575 → 23420、コメント行 3681 → 3608、コード行 16829 → 16759。`.xaml` は 29 ファイル・5733 → 5732 行（コメントブロック 287 → 285）。**行数削減率は成功指標にしていない。**
- **テスト**: `docs/development/test-suite-policy.md` に従い、`Assert.DoesNotContain("昔のhelper名", source)` のようなリファクタリング固定テストは追加していない。既存テストの削除・skip もしていない。変更したのは 2 箇所のみ: `ApplicationVersionTests` の version 文字列（`2.24.1` → `2.25.0`）と、`CH16OperationDiscoverabilityXamlTests` が入力エリアの範囲区切りに使っていた `"<!-- CH-4:"` を、XAML の実構造（削除確認オーバーレイの `IsDeleteConfirmVisible` バインディングを持つ `<Grid>`）へ変更したもの。判定範囲は同一で、コメント文言に依存しなくなった。同じ PR で `CH18MessageVisualHierarchyXamlTests` も、`"CH-18"` というコメントの存在確認だけを行っていた実装をやめ、`<DataTemplate x:Key="MessageTemplate">` の範囲に固定色リテラルがないことを実際に確認する形へ直した。いずれも「production のコメントを書き換えただけで CI が落ちる」結合を残さないための対応で、`docs/development/test-suite-policy.md` の方針に沿う。回帰は既存の behavior / compatibility / data protection テストで確認した。
...
- **docs**: `docs/development/nestsuite-development-guidelines.md` §3 へ「production source のコメント方針」を追記した（対象は `.cs` と `.xaml` の両方／WHY と制約を書く／version・backlog ID・PR 履歴は書かない／履歴は Git・release notes・planning docs へ置く／XAML の構造見出しラベルは残してよい／保存形式互換の記述だけは version 番号を含んでよい）。新規の長大な設計文書は作成していない。
...
- **テスト件数・実行時間（CI 実測）**: `dotnet test` の Total が **3637 → 3062（-575、Failed 0 / Skipped 0）**、実行時間は **13 秒 → 2 秒**（Test ステップの実時間は 64 秒 → 10 秒）。テストファイル数 198 → 184（削除 16・新設 2）。**件数削減自体は成功指標ではない**（成功指標は低価値な CI failure 要因が減り、重要な production contract が維持されていること）。
...
- **TN-3 / LK-4 / LK-3 / LK-2 の共存を確認した。** TN-3（TempNest → 新規 NoteNest タブ）が新規タブ生成・session 登録・rollback・`ActivateTab` を伴う一方、LK-4/LK-3/LK-2（既存タブへの追加）は共通ヘルパー（`EnumerateTransferTargets`/`TransferToWorkspaceTab`）だけを使う最小境界のままであることを再確認し、TN-3 を共通ヘルパーへ寄せない判断（TD-92 §15）が現在も妥当であると判断した。今回、TN-3 の共通化・リファクタリングは行っていない。
- **`WorkspaceTransferContent`（Title/Body の 2 フィールドのみ）・`WorkspaceTransferTarget`（WorkspaceKind + TabId + DisplayName）・`WorkspaceTransferResult`（Success/NoTarget/InvalidContent/TargetRejected/Failed の 5 値）の責務境界を再確認した。** 新しいフィールド・result 値は追加していない。転送先の識別は引き続き TabId を使用しており、表示名では識別していない。
- **`TransferToWorkspaceTab<TViewModel>` の責務が、入力検証・TabId+WorkspaceKind による対象解決・ViewModel 解決・accept delegate 実行・結果返却・予期しない例外の ErrorLog 記録だけに留まっていることを確認した。** ユーザー向け文言・Dialog 表示・Save・SaveAs・ファイル I/O・session 保存・ActivateTab・dirty 直接操作・転送元削除は共通ヘルパーに一切持たせていない。
- **0/1/複数タブ・Detached の契約を確認した。** LK-4/LK-3/LK-2 いずれも、転送先タブが 0 件なら自動生成せず案内のみ、1 件なら選択 UI なしで直接転送、複数件なら `WorkspaceTransferTargetDialog` で TabId ベースの明示選択（同名タブでも誤転送しない）。別ウィンドウ表示中（Detached）のタブも候補に含まれ、転送成功後も前面化・`ActivateTab` されない。
- **転送元保護・転送先 dirty 契約を確認した。** `NoTarget`/`InvalidContent`/`TargetRejected`/`Failed`/ダイアログキャンセルのいずれでも転送元は変更されない。TempNest は成功後に利用者が明示的に消去を選んだ場合のみ対象スロットを `Clear` する。転送先は成功時のみ、既存の追加経路（`CommitAdd`→`MarkDirty`／`AddNote`→`IsModified`）を通じて dirty になり、共通ヘルパーが `IsModified`/`HasChanges` を直接操作している箇所はない。
- **自動保存・自動タブ切替が行われないことを確認した。** LK-4/LK-3/LK-2 のいずれの導線にも `Save`/`SaveAs`/`AtomicFileWriter`/`File.Write*`/`SaveSessionAfterTabChange`/`ActivateTab` の呼び出しはない（TN-3 固有の session 処理は既存契約のまま維持）。
- **AutomationName / AutomationId 方針を確認し、2 件の取り残しを最小補正した。**
  - `WorkspaceTransferTargetDialog` の転送先一覧 `ListBox` の `AutomationProperties.Name` が、LK-4 実装時（IdeaNest 専用ダイアログだった当時）の固定文字列「転送先のIdeaNestタブ一覧」のまま、LK-2 で NoteNest 転送にも同ダイアログが再利用された後も更新されていなかった（NoteNest 転送時に読み上げ名が誤って「IdeaNest」になる）。`WorkspaceTransferTargetDialog` のコンストラクターへ `listAutomationName` パラメーターを追加し（既存の `windowTitle`/`promptText` と同じ形の任意パラメーター）、LK-4/LK-3 呼び出し側では従来どおり「転送先のIdeaNestタブ一覧」、LK-2 呼び出し側では「転送先のNoteNestタブ一覧」を明示的に渡すよう補正した。`AutomationId`・ダイアログの構成・キーボード操作・OK/キャンセルボタンは変更していない。
  - `TempNestWorkspaceView.xaml` の各スロットの「コピー」「クリア」「新規NoteNestへ昇格」ボタンが、SH-45（v2.19.14）・LK-4-1（v2.21.1）で確立した「可視テキストを持つボタンには `AutomationProperties.Name` を機械的に付与しない（WPF の既定どおり `Content` から導出させる）」方針に反し、`AutomationProperties.Name` へ内部 `AutomationId` 文字列（例: `TempNest.Slot1.CopyButton`）をそのまま設定していた（TN-3 実装時期の名残で、SH-45 の棚卸し対象がアイコン・記号のみボタンに限定されていたため見落とされていた）。同じ操作行内で LK-3/LK-2 が追加した「IdeaNestカードに追加」「既存NoteNestへ追加」ボタンは方針どおり `Name` 未設定だったため、行内で扱いが不統一になっていた。3 ボタン×4 スロット＝12 箇所の `AutomationProperties.Name` を削除し、`AutomationId`・`Content`・`Command`・`Click`・ToolTip・レイアウトは変更していない。
  - 上記いずれも表示・Command・挙動・保存契約への影響はない。
- **コード重複（LK-4/LK-3/LK-2 導線の 0/1/複数件分岐・`WorkspaceTransferResult` の switch 文の類似構造）を評価し、共通化しなかった。** 各転送の UX 差（通知文言・受入 delegate・TempNest 側の消去確認有無）が読みやすく、責務が明確で、変更頻度も低いため、具体的な不具合の重複修正や契約差異による回帰リスクは確認できなかった。新しい service・interface・DI・共通 CRUD・EventBus/Mediator は追加していない。
...
- **LK-5 相当（横断クイック投入等の汎用化）は実利用実績が観測されていないため、今回も着手しない。** LK-4/LK-3/LK-2 の 3 本が実装されている事実だけではトリガー成立扱いにせず、backlog の既存トリガー（実際に利用された実績）をそのまま維持した。新規 backlog 項目は推測で追加していない。
- **テスト**: `TD93WorkspaceTransferRegressionTests.cs` を新設し、共通型・共通ヘルパーの責務境界・LK-4/LK-3/LK-2 の Title/Body マッピング・0/1/複数件時の分岐契約・Detached 対応・転送元保護・dirty 契約・自動保存/自動タブ切替非実装・TN-3 の新規タブ生成/rollback/session 契約の維持・`WorkspaceTransferTargetDialog` の `listAutomationName` パラメーター・TempNest スロットボタンの `AutomationProperties.Name` 非設定・ErrorLog Error-only 方針・保存形式非変更・backlog/release-notes/設計文書の記録整合性を横断的に確認する。既存の `TD92WorkspaceTransferDesignDocsTests`/`LK4ChatNestToIdeaNestTransferTests`/`LK3TempNestToIdeaNestTransferTests`/`LK2TempNestToNoteNestTransferTests`/`TempNestTests` は削除・skip・弱体化していない。
- **実機でしか確認できない項目**（Windows 実機での TN-3/LK-2/LK-3/LK-4 の実操作、転送先 0/1/複数件・Detached・同名タブ・Titleのみ/Bodyのみ・元TempNest保持/消去・Dialogキャンセルの実確認、Narrator 等での AutomationName 読み上げ確認）は、本開発環境（Linux CLI、Windows 実機なし）では未確認のまま残っている。

---

## v2.23.0 — LK-2 TempNestスロット → 既存NoteNestタブへノート追加

- **LK-2: TempNest の各スロットに入力したタイトル・本文を、利用者の明示操作によって既存の NoteNest タブへ新規ノートとして追加できるようにした。** TN-3（TempNest → 新規 NoteNest タブへ昇格）とは別の導線であり、TN-3 の挙動は変更していない。設計正本は v2.20.1 / TD-92 の `docs/planning/workspace-manual-transfer-helper-design.md`。同設計を再検討・拡張せず、v2.21.0 / LK-4・v2.22.0 / LK-3 で実装済みの共通ヘルパーを実際に再利用して LK-2 のみを実装した（詳細は同文書 §24「実装結果（v2.23.0 / LK-2）」）。
- **LK-3/LK-4 で実装済みの共通ヘルパーをそのまま再利用した。** `WorkspaceTransferContent` / `WorkspaceTransferTarget` / `WorkspaceTransferResult` / `EnumerateTransferTargets` / `TransferToWorkspaceTab<TViewModel>` / `WorkspaceTransferTargetDialog` はいずれも変更・再実装していない。新しい共通転送サービス・`NoteNestTransferService`・`WorkspaceTransferManager` 等は作っていない。
...
- **NoteNest 受入 API を最小拡張した。** `MainViewModel.CreateNoteFromTransfer(string content)` の隣に `CreateNoteFromTransfer(string? title, string content)` を新規オーバーロードとして追加した。既存の 1 引数版は新オーバーロードへ `title: null` で委譲する薄いラッパーへ変更し、TN-3 が使う既存の挙動（本文からのタイトル生成・重複時の連番化・本文そのまま設定）は変更していない（回帰は既存 `MainViewModelPartialTests` の TN-3 テストで確認）。
- **タイトル方針**: TempNest の Title が指定されていればそれをそのまま新規ノートのタイトルとして使う。未指定（空／空白のみ）の場合は TN-3 と同じ既存のタイトル生成規則（`PromotedNoteTitleGenerator`）にフォールバックする。重複タイトルは既存の一意化ロジック（`MakeUniqueNoteTitle`）でそのまま連番化し（例:「タイトル (2)」）、新しいタイトル生成器・重複判定ロジックは作っていない。
- **LK-2 の導線（`NestSuiteShellWindow.TempNestToNoteNest.cs`）を新設した。** TempNest 側の入口は、各スロットの既存操作行（コピー／クリア／新規NoteNestへ昇格／IdeaNestカードに追加の並び）へ「既存NoteNestへ追加」ボタンを追加する形とし、新しい ContextMenu・常設ツールバー・設定画面は追加していない。TempNest 側は `TempNestSlotViewModel.TransferToNoteNestCommand` → `TransferToNoteNestRequested`（TN-3 の `PromoteRequested` / LK-3 の `TransferToIdeaNestRequested` と同じ形の delegate）で Shell へ要求する。追加した4つのボタンはいずれも可視テキスト（`Content="既存NoteNestへ追加"`）を持つため、`AutomationProperties.Name` は設定せず（v2.21.1 / LK-4-1 と同種の「内部 `AutomationId` をそのまま `Name` に設定してしまう問題」の再発防止。`AutomationId` は維持し、`AutomationName` は WPF の既定挙動どおり `Content` から導出させる）。ToolTip には「スロットのタイトル・本文を既存のNoteNestタブへ新規ノートとして追加します」を設定した。
- **既存の「NoteNestへ昇格」ボタン（TN-3）の表示文言のみ「新規NoteNestへ昇格」へ最小修正した。** 「既存NoteNestへ追加」（LK-2）との区別を明確にするための表記変更のみで、Command・Click ハンドラ・挙動・AutomationId・TN-3 の実装は変更していない。
- **NoteNest タブ 0/1/複数件の挙動は LK-3/LK-4 と同じ。** 0 件なら転送せず一時通知「NoteNest タブがありません。NoteNest を開いてから実行してください」のみで新規 NoteNest タブは自動生成しない（TN-3 とは別導線であるため）。1 件ならそのタブへ直接追加し「NoteNest「〈タブ名〉」にノートを追加しました」と通知する。複数件なら既存の `WorkspaceTransferTargetDialog`（新規ダイアログは作らない）で明示選択させ、選択対象は内部的に TabId で保持するため同名タブが複数あっても誤転送しない。別ウィンドウ表示中（Detached）の NoteNest タブも候補に含め、選択後も自動的に前面化しない。
- **転送成功後は TempNest に留まる。** `ActivateTab` は呼ばず、NoteNest へ自動タブ切替しない。転送は保存ではなく、共通ヘルパー・LK-2 導線のいずれもファイル I/O・自動保存・`SaveSessionAfterTabChange` を呼ばない。転送で dirty になった NoteNest は、既存の `AddNote` 経路を通じて自然に dirty 化するだけで、LK-2 コード側から `IsModified` を直接操作しない。利用者が通常操作（Ctrl+S 等）で保存するまでメモリ上の変更のまま残る。
- **転送成功後の元スロット処理は、TN-3・LK-3 と同じ考え方（既定「残す」の確認ダイアログ）で利用者に明示させる。** `DialogService.ConfirmWithSafeDefault` を再利用し、「はい」で対象スロットだけをクリア、「いいえ」（既定）で TempNest の内容を残す。失敗・キャンセル時（NoteNest タブなし・選択キャンセル・InvalidContent・NoTarget・TargetRejected・Failed）は元スロットを一切変更しない。
- **TN-3 は変更していない。** `NestSuiteShellWindow.TempNestPromotion.cs`・`PromoteTempNestSlotToNoteNest`・TN-3 の新規タブ生成・rollback・元スロット消去確認のいずれも今回のスコープ外。TempNest → 新規 NoteNest タブへの昇格の挙動は従来どおり。
- **LK-3・LK-4 は変更していない。** `NestSuiteShellWindow.TempNestToIdeaNest.cs`・`NestSuiteShellWindow.ChatNestToIdeaNest.cs`・それぞれの導線・挙動は今回変更していない。
- **LK-4・LK-3・LK-2 の 3 本で共通ヘルパーが実際に再利用された。** これを理由に新たな汎用化（LK-5 相当の横断クイック投入等）へは自動着手しない。転送導線が複数実装されたことに加え、実際に利用された実績が確認できた時点で初めて再評価する方針を維持する。
- **保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・IdeaNest / ChatNest / TempNest / session / draft / UI settings 形式への変更なし。外部依存の追加なし。EventBus/Mediator/DI 基盤の追加なし。転送履歴・マーク・双方向リンクの追加なし。**
- **テスト**: `LK2TempNestToNoteNestTransferTests` を新設し、Title/Body マッピング・余計な付加情報がないこと・`CreateNoteFromTransfer` 新オーバーロード（Title 指定時／未指定時のフォールバック／重複タイトルの一意化）・NoteNest タブ 0/1/複数件時の分岐契約・Detached 対応・転送後のクリア/保持双方・失敗/キャンセル時の TempNest 不変・NoteNest dirty 契約（既存経路のみ）・自動保存/自動タブ切替非実装・TN-3/LK-3/LK-4 の既存テスト維持・保存形式非変更を確認する。既存の `LK3TempNestToIdeaNestTransferTests` / `LK4ChatNestToIdeaNestTransferTests` は、LK-2 が backlog の open item から外れたことに伴うアサーション更新のみ行い、それ以外は削除・skip・弱体化していない。
...
- **マーカーの絞り込み対象**: マーカー本文（抜粋）・所属ノート名・マーカー種別（TODO/FIXME/NOTE の文字列）を対象に部分一致で判定する。既存の種別フィルタ（TODO/FIXME/NOTEチェックボックス）へAND条件で追加適用され、既存の`FilteredMarkers`（M15一括コピー・件数表示・空状態の正本）へ統合した。
- **タスクの絞り込み対象**: 現行の互換タスク一覧（`TaskGroups`）について、タスクタイトルの部分一致で判定する。絞り込み文字列がグループ名（「今日のタスク」等）自体に一致した場合は、そのグループ内の全タスクを表示する契約を採用した。未完了・完了の両方が絞り込み対象で、一致タスクが1件もないグループは非表示になる。
- **大文字・小文字・空白・特殊文字**: `StringComparison.OrdinalIgnoreCase`による大文字小文字を区別しない部分一致。絞り込み文字列の前後空白はTrimし、空白のみの場合は絞り込みなしとして扱う。`. * + ? [ ] ( ) \`等は正規表現として解釈せず、通常の`IndexOf`による文字列として扱う（新設の`RightPaneFilterMatcher`、既存の`SearchMatchSegments`等とは別に、右ペイン専用の薄いContains判定として追加）。
- **M15一括コピーとの連携**: マーカーは既存の`FilteredMarkers`をそのまま使うため、コピー処理側の変更なしに絞り込み後の項目だけがコピーされる。タスクは`TaskGroupViewModel.IncompleteTasks`/`CompletedTasks`を絞り込み対応にし、`NoteNestRightPaneMarkdownFormatter.FormatTasks`が受け取る一覧を画面表示と同一にした。タスクのコピー通知件数も、絞り込み後の実際のコピー件数と一致するよう修正した（従来は絞り込み前の総数を表示していた）。
- **空状態**: マーカーが絞り込みで0件になった場合、既存の「マーカーはありません」空状態と同じ表示枠を使い、元々0件かどうかで文言を切り替える（元々0件: 案内文言のまま、絞り込みで0件: 「一致するマーカーはありません」）。タスクは、タスクが存在し絞り込み中の場合にのみ「一致するタスクはありません」を表示し、元々タスクが0件の場合の既存空状態とは区別した。
- **クリア・Esc操作**: 入力欄の隣に「×」クリアボタンを配置し、絞り込み文字列がある場合のみ有効・表示する。クリック時は絞り込み欄を空にしてフォーカスを絞り込み欄へ戻す。絞り込み欄にフォーカスがある状態でEscを押すと、文字列がある場合のみクリアしてフォーカスを維持する（新しいグローバルショートカットは追加していない）。
- **タブ単位・別ウィンドウ**: 絞り込み文字列は`MainViewModel.RightPaneFilterText`が保持する。NoteNestタブは1タブ1`MainViewModel`インスタンスのため、絞り込み状態はタブごとに自動的に独立し、別ウィンドウ化しても同じ`MainViewModel`インスタンスをそのまま使うため状態を維持する。
- **dirty・保存形式・schema**: 絞り込み文字列の変更・クリアはNoteNestをdirtyにせず、本文・マーカー・タスクのデータやタスクの更新日時相当の状態を変更しない。絞り込み文字列自体もNoteNest保存データ・session・draft・UI settingsのいずれにも保存しない。NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・IdeaNest/ChatNest/TempNest保存形式への変更なし。
- **テスト**: `RightPaneFilterMatcherTests`（大文字小文字・日本語・特殊文字・Trim・空白のみ）、`NoteNestRightPaneFilterTests`（マーカー本文/ノート名/種別一致・既存種別フィルタとのAND条件・タスクタイトル/グループ名一致・空状態・タブごとの独立・dirty非影響・M15コピー連携の件数一致）、`NoteNestRightPaneFilterXamlTests`（絞り込み欄が1つだけであること・マーカー/タスク専用入力欄を追加していないこと・AutomationProperties/ToolTip・M15コピーボタンの維持・新しいグローバルショートカットを追加していないこと）を追加した。既存テストは削除・skipしていない。
- 実機でしか確認できない項目（実際の絞り込み反映速度・Tab/Enter/Escのキーボード操作・ライト/ダークテーマでの入力欄・クリアボタン・空状態の視認性・複数タブでの独立動作・別ウィンドウ化時の状態維持）は、本開発環境（Linux CLI、Windows実機なし）では未確認。

---

## v2.19.5 — ID-7 IdeaNest：検索語のカード内ハイライト

- **ID-7: IdeaNestのカード一覧で、検索語と一致した箇所をカードのタイトル・本文プレビュー内でハイライトするようにした。** 検索によって対象カードを絞り込む既存挙動はそのままに、どこが一致したのかを画面上で確認できるようにした。
- **対象範囲**: ハイライト対象はカード一覧内の「タイトル」「本文プレビュー」（固定行数モード・カードサイズS/M/Lに応じた自動高さモードの両方）のみ。タグ・色名・作成日・更新日・アーカイブ表示・プレビュー画面（`PreviewIdeaWindow`）・編集ダイアログ・Markdown出力・クリップボードコピーへは範囲を広げていない。
- **複数一致**: 同一のタイトル・本文プレビュー内に複数の一致がある場合、表示範囲内の一致箇所をすべてハイライトする。
- **既存検索仕様との整合**: ハイライト判定は既存の`FilterViewModel`の検索仕様（`OrdinalIgnoreCase`・前後空白Trim・通常の`IndexOf`/`Contains`相当、正規表現ではない）とまったく同じ条件を使う。日本語（ひらがな・カタカナ・漢字・全角英数字）・半角英数字はそのまま通常の文字列検索として扱う。タグだけが検索語に一致した場合、カードは検索結果に表示されるが、タイトル・本文に一致箇所がなければハイライトは出ない（正常動作）。
- **検索解除**: 検索語を削除すると、カードの再保存やWorkspace再読込なしに、その場でハイライトが消え通常表示へ戻る。
- **実装**: M17（v2.16.50、NoteNest全ノート検索のマッチ箇所ハイライト）で実装済みの3-Run分割helper（`FindReplaceLogicService.cs`の`SearchMatchSegments.Split`）を繰り返し呼び出す薄いラッパー`IdeaNestSearchHighlightHelper`（IdeaNest側、複数一致に対応するためのセグメント配列を返す）を追加し、同一のIndexOf分割ロジックは複製していない。表示は、カード表示対象文字列＋現在の検索語から読み取り専用の表示セグメントを作り`TextBlock.Inlines`／`Run`へ反映する添付ビヘイビア`IdeaNestSearchHighlightBehavior`（`Text`・`Query`添付プロパティ）で行い、検索語のバインディング変更で自動的に再計算される。カードモデル（`Idea`）・保存データへはRunや色などのUI情報を一切追加していない。ハイライト色は既存の`SearchHighlightBrush`（ライト/ダーク両テーマ定義済み、ChatNest検索ハイライトでも使用中）を`DynamicResource`で再利用し、新しいリソースは追加していない。
- **フォーカス・操作**: ハイライト対象のTextBlockへ`IsHitTestVisible="False"`を設定し、カードへのTab移動・Enterプレビュー・フッターボタン・右クリックメニュー・マウスクリック・ピン留め・アーカイブなど既存のBorderレベルのイベント配線には影響しない。
- **dirty・保存形式**: 検索語入力・ハイライト更新・検索解除はいずれもIdeaNestをdirtyにせず、カード本文・更新日時・保存データを変更しない。IdeaNest保存形式・`.nestsuite` wrapper（`formatVersion 1.0`）・NoteNest schema（`1.4.2`）・ChatNest/TempNest/session/draft/UI settings形式への変更なし。
- **非対象**: 検索ロジック・検索対象・検索結果件数・検索欄・検索ショートカット・タグフィルタ・アーカイブフィルタ・ソート・シャッフルは変更していない。タグ・プレビュー画面・編集ダイアログへのハイライト拡大、一致箇所への自動スクロール、本文プレビューの切り出し位置変更、複数検索語・正規表現検索・検索履歴・ハイライト色設定、RichTextBox化・独自描画エンジンの導入は行っていない。外部依存の追加なし。
- **テスト**: `IdeaNestSearchHighlightHelperTests`（検索語なし／一致なし／先頭・中央・末尾・全文一致／複数一致／重なり一致の非重複前進IndexOf契約／大文字小文字／日本語／特殊文字を正規表現として解釈しないこと／空白Trim／タグだけ一致時にタイトル・本文がハイライトされないこと）、`IdeaNestSearchHighlightXamlTests`（タイトル・本文プレビュー2箇所がbehaviorを利用していること・タグ表示とプレビュー画面には拡大していないこと・RichTextBox/FlowDocument/WebViewを追加していないこと・既存3-Run helper再利用の静的確認）を追加した。既存テストは削除・skipしていない。
- 実機でしか確認できない項目（複数カード・複数一致での実際のハイライト表示、タグだけ一致時の見た目、カードサイズS/M/L・ライト/ダークテーマ・カード背景色ごとの視認性、Tab/Enter/マウス/右クリック/ピン留め/アーカイブの操作回帰、保存済みIdeaNestを開いて検索・ハイライトのみ行った場合の未保存表示の有無）は、本開発環境（Linux CLI、Windows実機なし）では未確認。
...
- **結果: 残す21件／見送り・削除・整理18件。** 残す21件の内訳は、通常エンジニア向け（着手トリガー不要）7件（L4・L10・M15・ID-4・ID-7・TD-83・TD-87）、トリガー待ち14件（M13・ID-8・ID-12・CH-12・LK-2/3/4・TD-85・LT-2/3/4/6/9/11）。全残存項目に観測可能な着手トリガー（または「トリガー不要」の明示）と実装主体を付記した。
- **主要な分類変更**: SH-24・TN-4・M7・M8・ID-5・ID-13・CH-17・LK-5・LT-1・LT-5・LT-7・LT-8・LT-10を見送り（欠番、再提案時は新ID）。H3はRJ-11（エディタ差し替え前提の視覚拡張の恒久見送り）へ移動。TN-7はLK-2/LK-3へ、LK-1はLT-6へ吸収（重複解消）。SH-35はLT-9フェーズ2を正本とする参照ポインタへ整理（完了扱いにはしない）。LT-12は最小実装済みのため役割終了。TD-87は予約状態を解消し通常エンジニア向けの正式な行として登録した。ID-4の優先度をB→Aへ引き上げ（TD-88で範囲固定済みのため）。LT-2とLT-6の循環参照は共通トリガー「SH-41方式横断検索の実測限界」へ一本化した。
- **エキスパート再招集が必要な項目: 現時点0件。** 将来トリガー成立時のみLT-6（＋LT-2）・LT-3・LT-4が対象。M18・AT-1/2/3フェーズ2・LT-9フェーズ2など確定済みの判断は再検討していない。
- **実装なし**（本versionの変更はレビュー文書・backlog・release notes・version・docs整合テストのみ。プロダクトコード変更なし）。
- backlog運用ルール（着手トリガーのない長期候補を追加しない・欠番IDを再利用しない・実装主体と設計レビュー状況を明記する・実利用評価が必要な項目を推測で着手しない等）を`docs/backlog.md` §2へ追加した。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・session形式・recent files形式・draft形式・TempNest形式・IdeaNest形式・ChatNest形式・UI settings形式の変更なし。外部依存追加なし。
...
- **確認環境の制約**: 本セッションの実行環境はLinux CLIサンドボックスであり、Windows実機・WPF GUIを直接操作できない（`dotnet build`/`dotnet test`もこの環境では実行不能）。そのため点検は、実装コードの静的読解による再監査・既存テスト内容の確認・GitHub Actions（windows-latest）でのbuild/test/ui-smoke実行を組み合わせて行った。実際の目視・マウス・キーボード操作を要する確認項目は「未確認」として記録し、無理に「確認済み」とはしていない。
- **発見・修正した不整合（3件、いずれも表示文言のみ）**:
  1. IdeaNestの削除確認ダイアログ文言「他の操作を行うと元に戻せません」が、実際のUndo無効化条件（新しい削除・アーカイブ操作／別ファイル読込／Workspace破棄のみ）より広く読め、誤解を招いていたため、「削除直後なら「元に戻す」で取り消せます。次の削除・アーカイブ操作を行うと、以前の操作は元に戻せません。」へ修正した（ID-6の機能・確認フロー自体は変更していない）。
  2. IdeaNestの「ファイル→エクスポート」メニューが、ID-10実装後も空実装当時の文言「Markdown風テキスト」のまま残っていたため、「Markdownとして保存」へ修正した（AT-2の既存語彙と統一。コマンド・配置は変更していない）。
  3. SH-40「続きから」導線がuser guideに未記載だったため、「最近使ったファイル」節の直後に説明を追加し、SH-41「最近のファイルも検索」との役割の違い（ファイル名で再開 vs 内容で検索）を明記した。
- **AT-5・SH-40の排他、SH-40・SH-41の役割分担、ID-6 Undoの無効化条件、ID-10のMarkdown出力範囲、SH-41の既定OFF・ON時1回読込・dirty優先・cancellation・100件上限**について、実装コードと既存テストの内容を再確認し、上記3件以外の明白な不整合は見つからなかった。
- **未確認事項**: 実際の目視・操作を要する確認（画面レイアウト・フォーカス移動・テーマ表示・ネットワークパスでの体感遅延・UIフリーズの有無・メモリ使用量の実測等）は、本セッションの環境制約により未確認のまま`docs/testing/attractiveness-regression-v2.18.19.md`に明記した。次回Windows実機での確認を推奨する。
- **今後の判断事項**（backlogへは追加せず、次回の開発判断時の材料として記録するのみ）: SH-40のTempNest入力後の表示維持方針の見直し要否、SH-40/SH-41の表示・検索件数の妥当性、同名ファイルの判別しやすさ、ネットワークパス時の読込遅延、SH-41の実機性能計測の要否。
- テスト: `IdeaNestWorkspaceViewModelTests.cs`へ削除確認文言の整合確認（新規）、`ID10MarkdownExportXamlTests.cs`のメニュー文言確認を新文言へ更新、`NestSuiteDocsContractTests.cs`へuser guideのSH-40記載確認（新規）を追加した。既存テストの削除・skipはしていない。
- backlog: SH-42を完了済み欠番としてbacklog.mdへ反映した（新規追加ではなく、既存の完了済み一覧文への追記）。AT-1・AT-2フェーズ2、一致箇所への移動、recent 10件化、LT-2、TD-85等は新規backlogへ追加していない。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session形式・recent files JSON形式・TempNest形式・IdeaNest形式・ChatNest形式の変更なし（今回の修正はUI文言・docsのみ）。UI settingsへの新規フィールド追加なし。外部依存追加なし。

---

## v2.18.18 — SH-41 / AT-2 フェーズ1 最近使った未オープンファイルの横断検索

- **SH-41 / AT-2 フェーズ1: Shell横断検索に「最近のファイルも検索」を追加し、利用者が明示的にONにした場合だけ、最近使った未オープンファイルの内容も検索対象に含められるようにした。** 魅力向上エキスパートレビュー（`docs/planning/attractiveness-review-2026.md` AT-2「最近使った未オープンファイルの横断検索」）のフェーズ1対応で、設計根拠は`docs/planning/at2-recent-unopened-cross-search-design-review.md`。AT-2全体（一致箇所への移動等のフェーズ2）は今回完了していない。
- **既定はOFF**。既定の横断検索（開いているタブのみ、キー入力ごとの即時検索）は変更していない。「最近のファイルも検索」チェックはパネルを閉じると次回はOFFへ戻り、ON/OFF状態は永続化しない。
- **ON時だけ、現在開いていないrecent files上位5件を1回だけ非同期で読み込む。** 開いているファイルは既存の同一ファイル判定（`NestSuiteOpenFilePolicy.IsSameFile`）で候補から除外する。キー入力のたびにファイルを読み込むことはせず、読込完了後はメモリ上のスナップショットへキー入力ごとに検索する（既存の検索体験を維持）。
- **Workspace ViewModelは生成せず、既存の`NestSuiteTabFactory.TryPrepareOpen`と各`*FileService.LoadPrepared`（NoteNest/IdeaNest/ChatNestで既に確立済みの、UI非依存・副作用なし・schema検証込みの読込経路）だけを再利用して保存モデルまで読み込む。** 検索専用のReaderは新設していない。
- **検索対象はNoteNest（ノートタイトル・本文）・IdeaNest（カードタイトル・本文・タグ、アーカイブ含む）・ChatNest（発言本文）で、開いているタブ検索と同じ項目・同じ一致仕様（`OrdinalIgnoreCase`の部分一致）。** TempNestはrecent fileにならないため未オープン対象に含めていない。
- **結果は「開いているファイル」「最近使ったファイル」の2グループで表示し、開いている結果を先に表示する。** 0件のグループは見出しごと表示しない。全体の結果上限100件は両グループで共有し、開いている結果を優先する。
- **未オープン結果のクリックは、最近使ったファイルメニューと共有する既存open経路（`ShellFileOpenPlanner.Plan`）でファイルを開き対象タブをアクティブ化するところまで。** 一致したノート・カード・発言への自動移動は実装していない（開いているタブの検索結果も同様、フェーズ2で判断）。開いた後は次回検索から開いているタブ側へ統合され、未オープン側との重複は生じない。
- **非同期・キャンセル**: 読込はバックグラウンドで実行し、UIスレッドをブロックしない。最大5件は原則逐次読込。チェックOFF・パネルを閉じる・Shell終了・再ON時はキャンセル要求を出し、古い読込結果はUIへ反映しない（ファイルの合間でキャンセルを確認するため、1ファイルの同期読込そのものを即座に中断できるわけではない）。
- **読込失敗（不存在・破損・未対応形式・schema too-new等）は該当ファイルだけスキップし、検索全体は継続する。** 個別のエラーダイアログは出さず、パネル内へ件数だけ表示する。recent filesからの自動削除は行わない（一時的な障害と不存在を区別できないため）。
- **キャッシュはパネル表示中かつON中だけのメモリ上スナップショット。** OFF・パネルを閉じる・Shell終了で破棄し、永続化しない。ファイル監視・更新日時キャッシュ・アプリ起動中キャッシュは実装していない。
- **情報露出**: 未オープンファイルの本文断片は明示的にONにした場合だけ表示される。フルパスはToolTipのみ、断片は既存の一致行抜粋（最大120文字）と同じ仕様。検索語・検索結果・チェック状態は保存しない。
- **AT-1（SH-40）との境界**: SH-40「続きから」のXAML・キャッシュ・recent表示件数（3件）は変更していない。検索ボタン等の新しい導線もSH-40側へは追加していない。recent filesキャッシュ・同一ファイル判定・`OpenRecentFile`は共有する。
- テスト: `ShellSearchServiceTests.cs`（拡張）で未オープン候補選定（MRU順維持・上位5件・開いているファイル除外・大文字小文字/null考慮）と未オープン検索（NoteNest/IdeaNest/ChatNestの各対象項目・アーカイブカード含む・日本語・大文字小文字非区別・予算内切り詰めと切り詰め有無の判定）を確認。`UnopenedRecentFileLoaderTests.cs`（新規）で`.nestsuite`・legacy拡張子読込、Workspace ViewModelを生成しないこと、不存在/未対応形式/破損JSONのスキップ、1件失敗が他ファイルを止めないこと、注入した読込関数の呼出回数で実ファイルI/Oに依存しないことを確認。`ShellSearchPanelViewModelTests.cs`（拡張）でON時の1回読込とSearchText変更時に読込が増えないこと、開いているファイルの候補除外、open後の未オープン側からの除外（スナップショット再読込なし）、OFF/Reset時のクリア、読込中表示、キャンセル時に結果が反映されないこと、Dispose時のキャンセル、開いている結果優先の100件予算配分を確認。`SH41CrossSearchUnopenedTests.cs`（新規）でチェックボックス・グループ見出し・AutomationId・ToolTip・新規固定色なしの静的UI契約を確認。既存テスト（SH-15系横断検索テスト）の削除・skipはしていない。
- `docs/guide/nestsuite-user-guide.md`・`docs/design/nestsuite-known-limitations.md`を実装内容に合わせて更新した。
- backlog: SH-41を完了済み欠番としてbacklog.mdへ反映した（新規追加ではなく、既存の完了済み一覧文への追記）。AT-1・SH-40は変更していない。一致箇所への移動（AT-2フェーズ2）・SQLite・recent 10件化・並列読込は新規backlogへ追加していない。LT-2は保留継続のまま変更していない。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session形式・recent files JSON形式・各Workspace保存形式の変更なし（「最近のファイルも検索」の状態・読込スナップショットはメモリ上のみで、いずれの保存データにも含まれない）。UI settingsへの新規フィールド追加なし。外部依存追加なし。
...
- **発言者ラベル**: フォントサイズ（11px・Bold）は本文（13px）より小さく太いという既存の関係をそのまま維持した。発言者ラベル行（チップ＋時刻）の下マージンを`0,6,7,0`から`0,6,7,4`へ広げ、本文との間にわずかな間隔を設けた。発言者色（`SpeakerBg`/`SpeakerAccent`）・チップ形状・左右配置・発言者名の表示（連続発言時も省略しない）は変更していない。
- **メッセージ間の余白・カード境界**: 通常のメッセージ間余白（`Margin="16,3"`）は変更せず、CH-11の日付区切りの余白のみ`0,12,0,6`から`0,16,0,8`へ広げ、区切りが会話全体の節目であることを、メッセージ間の余白より一段広い間隔で伝えるようにした。メッセージ本文の吹き出しは、発言者アクセント色の左ボーダー（3px）に加えて上下右にも極細（1px）のボーダーを追加し、カード境界をわずかに補強した（強い枠線・影・角丸の変更はしていない）。
- **入力欄との境界**: 入力欄パネルの上端に、検索バー下端の境界線（`BorderThickness="0,0,0,1"`）と対になる`BorderThickness="0,1,0,0"`を追加し、`{DynamicResource BorderBrush}`で会話一覧との境界を明確にした。入力欄の幅・パディング・送信ボタン・発言者選択UIは変更していない。
- **選択中メッセージ**: 調査の結果、ChatNestには現時点で「選択中メッセージ」という状態自体が存在しない（`IsSearchCurrent`＝検索中の現在位置、`IsDragging`＝ドラッグ中、`IsEditing`＝編集中のみが既存の表示状態）。存在しない機能を新設すると「メッセージ選択機能を拡張しない」という方針に反するため、この項目は対象外とし、状態を新設していない。
- **状態の優先順位**: 既存のTrigger構成を確認した結果、`IsSearchCurrent`（Background）と`IsDragging`（Opacity）は異なるプロパティに作用するため同時発生時も競合せず、`IsEditing`は本文/編集欄の表示切替（Grid内の別要素）、削除確認（`IsDeleteConfirmVisible`）はメッセージ一覧全体を覆う最前面オーバーレイのGridであるため、いずれも自然に共存しており新たな優先順位の整理は不要と判断した。CH-11の日付区切りには、選択・検索・ドラッグのいずれのTriggerも従来通り適用されない。
- テスト: `CH18MessageVisualHierarchyXamlTests.cs`（新規）で、発言者ラベルのマージン・フォント関係、日付区切りの余白、メッセージ本文のBorderThickness、入力欄の上端境界（DynamicResource使用・新規固定色なしを含む）、既存ContextMenu・AutomationId・IsSearchCurrent/IsDraggingトリガーが日付区切りへ適用されないことを確認した。ViewModelおよび保存ロジックは変更していないため、対応する単体テストの追加はしていない。既存テストの削除・skipはしていない。
- backlog: CH-18を完了済み欠番としてbacklog.mdから削除した。CH-16・CH-17は未実装のまま、優先度も変更していない。CH-11は「再実装済み」等への変更をしていない。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session形式・ChatNestの既存保存形式・timestampの既存保存形式・発言者形式・メッセージの保存順の変更なし。外部依存追加なし。
...
- **件数の定義**: 現在の検索・タグ・アーカイブ条件を反映したうえで、色フィルタを適用する直前のカード集合に含まれる、その色のカード数。各色チップを選択した場合に表示されるカード数と一致する。
- **自己フィルター除外**: 現在選択中の色フィルタ自身は件数計算から除外する。`FilterViewModel.Apply`（アーカイブ→タグ→検索語→色の順）から、色条件を適用する直前の段階を`ApplyExceptColor`として切り出し、色を選択中でも他色の件数が0件にならないようにした。
- 表示形式は、現行の色チップ（22×22の色付き丸のみで色名を常時表示しないUI）へ小さな件数テキストを併記する形（案A）を採用した。ツールチップは「赤：5件」の形式で色名と件数の両方を確認できる。0件の色も表示し、3桁件数（例: 128）でもレイアウトは崩れない。
- 更新タイミング: Workspace読込・カード追加・削除・色変更・アーカイブ／復元・検索語変更・タグフィルタ変更・アーカイブ表示切替・全フィルター解除で自動的に更新される。既存のカード変更→再描画の一元経路（`RefreshVisible`）に相乗りしたため、更新漏れの新しい経路を追加していない。
- 件数は保存しない派生状態（`ColorFilterItemViewModel.Count`）として実装した。カード保存JSON・`.ideanest`/`.nestsuite`既存形式へは一切混入しない。
- 選択中チップの既存強調（枠線・Hover・キーボードフォーカス・ダーク/ライトテーマ追従）は変更していない。件数テキストは既存の補助文字色`IdeaTextSecondaryBrush`（DynamicResource）を使い、新しい固定色は追加していない。
- 既存の色名・ツールチップテキストのAutomationProperties.Nameを、色名のみから「色名、件数」形式へ拡張した。AutomationId自体は変更していない。
- テスト: `IdeaNestColorFilterCountTests.cs`（新規）で、基本件数・自己フィルター除外・検索/タグ/アーカイブ条件の反映・カード追加/削除/色変更/アーカイブによる更新・`IdeaNestWorkspaceViewModel.ColorItems`への反映・フィルター結果との一致・表示文字列（0件/3桁）・保存JSONへ件数が混入しないことを確認した。`ID14ColorFilterChipXamlTests.cs`（新規）でチップのXAML契約（件数表示・ツールチップ・AutomationName・DynamicResource・選択強調の維持）を静的に確認した。既存テストの削除・skipはしていない。
- backlog: ID-14を完了済み欠番としてbacklog.mdから削除した。ID-13（簡易統計）は実装済み扱いにせず、ID-4・ID-5・ID-6等の優先度、M14・M18・TN-7の状態は変更していない。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session形式・IdeaNestの既存保存形式・カード色の既存保存形式の変更なし。外部依存追加なし。

## v2.18.7 — L24 タスクの関連ノート選択を一覧化

- **L24: タスクへ関連ノートを設定する際、ノート名を覚えて文字列で完全入力する必要をなくした。** タスク機能の再拡張ではなく、既存互換機能（縮退方針 TD-52 対象）の入力負荷軽減のみを目的とする。タスクの露出増加・新規タスク管理機能・複数ノート関連付け・ノートID保存化は行っていない。
- 右クリックメニュー「関連ノートを設定...」は、従来はノート名を1件ずつ手入力し `FindNoteByTitle` で解決していた（入力ミスに気づきにくく、ノートブックも分からない）。これを既存の `NotePickerDialog`（ノートリンク挿入で使用しているものと同じダイアログ）を再利用した一覧選択に置き換えた。L24専用の新規ダイアログは作成していない。
- タスク編集パネルの関連ノート欄（`RelatedNoteChoices` ComboBox、ノート名のみ表示）はそのまま維持しつつ、隣に「選択...」ボタンを追加した。同じ `NotePickerDialog` を開き、ノートブック名付きの一覧・絞り込みで選べる。
- 候補は「ノートブック名 / ノート名」で表示する（例: 「庁内DX / 端末更新」）。絞り込みはノート名・ノートブック名・結合表示文字列のいずれかで一致し、大文字小文字を区別しない部分一致。0件時は「該当するノートはありません」を表示する。
- 既にタスクへ関連ノートが設定されている場合、ダイアログを開いた時点で対応するノートを初期選択する（`LinkedNoteId` から一意に解決できる場合のみ。曖昧・未検出の場合は自動選択しない）。決定した場合だけ関連ノートへ反映し、キャンセル時は元の値を維持する。
- 保存値は現行どおり `TaskViewModel.LinkedNoteId`（`NoteViewModel.Id` による一意識別）のままで、ノートブック名や複合形式・独自内部マッピングは保存していない。同名ノートが存在しても、選択操作自体は実際の `NoteViewModel` インスタンスを対象にするため曖昧性は生じない（`[[タイトル]]` リンク挿入とは異なり、関連ノートはタイトル文字列ではなく一意な `Id` で解決されるため）。
- 関連ノートを開く操作・クリア操作（右クリックメニュー・編集パネルのボタン）は変更していない。既存の空文字/未設定による関連解除の挙動もそのまま。
- テスト: `NotePickerFilterServiceTests.cs` に絞り込み拡張（ノート名・ノートブック名・結合表示文字列・大文字小文字・部分一致・0件）のテストを追加。`L24RelatedNotePickerXamlTests.cs`（新規）で、選択ボタン・既存ComboBoxの維持・空状態表示・既存AutomationIdの維持・`NotePickerItem.DisplayText`の形式を静的に確認した。タスク編集View/Dialogのハンドラ自体は既存方針（WPF View/Windowの直接構築はユニットテスト対象にしない）に従い手動確認で検証した。既存テストの削除・skipはしていない。
- backlog: L24を完了済み欠番としてbacklog.mdから削除した。タスク縮退方針・L10・M15・M18・TN-7の状態は変更していない。M7（ノートブック名修飾）の先行実装はしていない。
- `docs/planning/review7-fable5.md`は本文を変更せず、末尾に「L24はv2.18.7で対応済み。」の追記のみ行った。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session形式・タスクの既存保存形式・関連ノートの既存文字列形式（Id）の変更なし。外部依存追加なし。
...
- 指摘件数: **Blocking 0 / High 0 / Medium 2 / Low 2 / Observation 複数**。Medium は REV7-1（TN-3 昇格直後に `ActivateTab` へ古いタブ record が渡り、タブストリップの選択表示が新規タブへ移らない）と REV7-2（既存 NoteNest タブで別ファイルを開いた直後、L23 の空状態ガイドが古い表示のまま残る。`NoteWorkspaceViewModel.Load` の suppress 後に `Changed` 未発火）。いずれも表示のみの不整合で、データ消失・保存経路への影響はない。
- 総合評価: 魅力向上フェーズは方針（明示操作・読み取り専用・保存形式不変・既存状態の再利用）を守れており、テーマ継続を推奨。データ消失につながる新しい経路はなし。
- **次に着手すべき 1 件: SH-38 + L25 の小規模修正バージョン（v2.18.5 候補）。** その後の機能実装第 1 候補は M19（UI 設定・最近使ったファイルの復旧経路。最小範囲をレビュー §5.1 に定義）。M18（XAML 分割）は変更頻度が低くトリガー待ち・着手前に専用設計レビュー要。TN-7 は TN-3 実利用フィードバック待ちを維持。L24 はタスク縮退方針との釣り合いから優先度 A→B へ調整。
- backlog 更新: SH-38（REV7-1 修正）・L25（REV7-2 修正）を新規追加（2 件）、L24 の優先度を A→B へ調整。完了扱いへの変更・LT-9 フェーズ2/3 や SH-35 の状態変更はなし。
- `docs/README.md` の planning 索引へレビュー文書への導線を追加し、docs-contract テストへ文書の存在・主要内容の確認を最小追加した。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session 形式・各 Workspace 保存形式の変更なし。外部依存追加なし。
...
- 作成カードは `PreviewIdeaWindow.AddedCard`（`CardOperationsService.CommitAdd` の戻り値そのもの）で明示的に識別する。タイトル・作成日時・一覧上の位置・ソート順からの推測は行わない。同一タイトルのカードが複数あっても正しいインスタンスを対象にする。
- 作成カードが現在の表示条件（検索・タグ・色・アーカイブ表示）を満たし `VisibleCards` に含まれる場合は、そのカードを一時選択状態にし（`IdeaNestWorkspaceViewModel.SelectedCard` / `IdeaCardViewModel.IsSelected`）、既存のキーボードフォーカス強調と同じ `IdeaAccentBrush` の枠線で示す。表示領域外の場合のみ、コンテナ生成後に `BringIntoView` で 1 回だけスクロールする（固定時間の待機・再試行タイマー・LayoutUpdated監視は使わない）。既に表示されている場合は不要なスクロールを行わない。
- 表示条件に含まれない場合（検索文字列・タグ・色・アーカイブ条件に一致しない等）は、フィルター・検索文字列・アーカイブ表示状態を一切変更せず、「カードを追加しました。現在の絞り込み条件では表示されていません」と既存の一時ステータス通知（`ShowStatus`）で短く知らせる。表示対象内では、選択と枠線で十分に分かるため追加の通知は出さない。
- 表示対象判定は新規コードで再判定せず、既存の `FilterViewModel.Apply` 済みコレクションである `VisibleCards` をそのまま基準にした。
- 選択状態はカード作成専用の一時状態であり、`Idea` モデル・`.ideanest` 保存形式・session/draft 形式には一切含めない（`ID-9` 由来の既存回帰テストで hover/focus 同様に保存混入がないことを確認済み）。複数選択（ID-5）の先行実装ではなく単一選択のみ。
- 連続して作成した場合は最後に作成したカードだけが対象になる。作成失敗・ダイアログキャンセル時は選択・スクロール・通知のいずれも行わない。ワークスペースの再読込（別ファイルを同じタブで開く等）では選択状態を持ち越さない。ソート変更や他カードのピン留め切替など、新規作成以外の一覧更新では選択を変更しない。
- ID-4（キーボード操作）・ID-5（複数選択）・ID-6（Undo）・ID-7（検索語ハイライト）は今回実装していない。IdeaNest 以外の Workspace への横展開も対象外。
- テスト: `IdeaNestNewCardPositionFeedbackTests.cs` を新規追加し、作成カードの識別・表示対象判定（検索/タグ/色/アーカイブ）・選択・スクロール要求発火・連続作成・ワークスペース再読込時のクリア・既存カード操作との非干渉・XAML静的確認を検証した。既存テストの削除・skip はしていない。
- backlog: ID-15 を完了済み欠番として backlog.md から削除し、ID-4・ID-5・ID-6・ID-7 等の他項目は変更していない。
- 保存形式・NoteNest schema（`1.4.2`）・`.nestsuite` wrapper（`formatVersion 1.0`）・`draftFormatVersion 1.0`・session 形式・IdeaNest 保存形式の変更なし。外部依存追加なし。
...
- `docs/archive/completed-designs/README.md` と `docs/archive/migrations/README.md` を追加し、各 archive フォルダー固有の役割と、現行仕様・運用の正本ではないことを明記した。
- `docs/integration/README.md` / `docs/migration/README.md`、Markdownリンク、docs棚卸し結果を移設後の状態へ更新した。
- TD-79 の backlog 残存行がないことを確認し、TD-80 を完了済み欠番として記録した。次候補として TD-81（static-test・spike系文書のarchive移設）を追加したが、今回は着手していない。
- 文書本文の内容改訂は行っていない。production コード変更なし、UI 変更なし、保存形式変更なし、session.json 変更なし、Workspace 保存形式変更なし、NoteNest schema 変更なし、wrapper 変更なし、draft format 変更なし。外部依存追加なし、net48_test 再開なし。
...
- 設計者向け・開発者向け両ガイドラインの役割分担を明文化した。開発者向けガイドラインは実装者向けの正本、設計者向けガイドラインは設計・課題選定・プロンプト作成・引継ぎの正本として分離した。
- 引継ぎプロンプトと恒久規約を分離した。引継ぎプロンプトには現在地・確定判断のみを記載し、恒久規約は両ガイドラインを参照する運用とした。
- 個別実装プロンプトを規約との差分中心にする方針を復活させた。開発者向けガイドラインに既存の共通事項は個別プロンプトへ全文再掲しない。
- `docs/development/nestsuite-development-guidelines.md` に設計者向けガイドラインへの相互参照を追加した（既存の開発ルール本文は変更していない）。
- production コード変更なし、UI 変更なし、session.json 変更なし、Workspace 保存形式変更なし、NoteNest schema 変更なし、wrapper 変更なし、draft format 変更なし。外部依存追加なし、net48_test 再開なし、既存テスト削除・skip なし。
...
- ChatNest sidecar の `SelectedSpeaker` は既存 `Speaker` enum 名へ正規化し、null・空・空白・未知・数値・大文字小文字違い等の不正値は `自分` へフォールバックする。Speaker 値だけの異常では `Loaded` を維持する。
- `DraftStore` の `UnsupportedVersion` / `IoError`、Speaker 正規化、sidecar 単独隔離、HashMismatch sidecar 隔離、隔離先衝突、正常フルパス tabId 抽出の回帰テストを追加した。
- 3 FileService の `SerializeWrapped` / `.nestsuite` Save 一致 / legacy 保存形式非 wrapper 化の回帰テストを追加し、IdeaNest serialization の入力 Workspace 無副作用も固定した。
- NoteNest / IdeaNest / ChatNest の snapshot 無副作用テストと、SaveAs / CloseTab / OnClosing / timer 二重起動防止に関するライフサイクル contract test を補完した。
- SH-36b は v2.16.45 で起動時復元予定。NoteNest schema は `1.4.2` のまま、`.nestsuite` wrapper `formatVersion` は `1.0` のまま。session.json・Workspace 保存形式の変更なし。外部依存追加なし。既存テストの削除・skip なし。

## v2.16.43 — SH-36a: 無題タブの下書き自動保存（書込側）

- 無題の NoteNest / IdeaNest / ChatNest タブについて、既存 30 秒 tick を使い `%APPDATA%\NoteNest\drafts\` へ下書きを書き込む書込側を実装した。候補 true では `draft-<tabId>.nestsuite` を atomic write し、候補 false では現在開いている該当 tabId の active pair だけを削除する。
- ChatNest は `.nestsuite` 本体に確定済みメッセージを保存し、sidecar `draft-<tabId>.state.json` に `InputText` / `SelectedSpeaker` / `EditingMessageId` / `EditingText` と本体 SHA-256 を保存する。
- Workspace 別 dirty 判定は NoteNest = `IsModified`、IdeaNest = `HasChanges`、ChatNest = `HasUnsavedChanges` とし、下書き保存では dirty・FilePath・session・recent files を変更せず、保存完了通知も出さない。`.bak` は作らない。
- 3 FileService にファイル I/O なしの `SerializeWrapped` を追加した。`.nestsuite` Save と同じ wrapper を返すが、legacy `.notenest` / `.ideanest` / `.chatnest` 保存形式は従来どおり payload のまま維持した。
- SaveAs の状態反映成功後、タブ閉鎖確定後、正常終了確定後に active 下書き pair を削除する。OnClosing 冒頭で auto-save timer を停止し、Cancel 経路では再開する。
- SH-36a では自動復元 UI は実装しない。手動復旧は drafts フォルダーの `draft-*.nestsuite` を通常の「ファイル → 開く」で開く。ChatNest sidecar は JSON として人が確認できるが、通常の「開く」では反映されない。SH-36b は v2.16.44 で起動時復元予定。
- sidecar 読込結果は NotPresent / Loaded / InvalidFormat / UnsupportedVersion / HashMismatch / IoError に分類する。HashMismatch sidecar は削除せず、隔離して内容を保持する方針へ訂正した。
...
- 挙動テスト化候補 2 件（`IsSessionPersistable` の出力確認化、コンストラクター SaveSession 条件の policy helper 化）を特定した
- フォローアップを TD-75 として backlog に追加した
- 今回は分類レビューのみで、既存テストの削除・skip・実装変更は行っていない
- コード動作・session 形式・保存形式・schema・wrapper 変更なし

## v2.16.24 — LT-9: フェーズ2設計判断の backlog 反映

- review5-fable5.md の LT-9 フェーズ2設計レビュー結果を backlog に反映した
- LT-9 フェーズ2は、設計判断を固めたうえで、現時点では実装しない方針を記録した
- 着手トリガーとして、FileNotFound all-or-nothing 解除の実害報告、InvalidFormat 等の恒久 nag の実害報告、SH-35 解除対象拡張の実装判断を記録した
- フェーズ2実装時は、失敗 entry 2 件以上の場合のみ小さな選択 UI を出し、1 件の場合は SH-34 の MessageBox 経路を維持する方針を記録した
- フェーズ2実装時は、全失敗種別を解除対象に含めるが、初期値は未チェック、すべて選択なし、安全側既定とする方針を記録した
- session.json 形式変更は不要であり、失敗理由は保存せず起動時復元試行で得る方針を再確認した
- LT-9 フェーズ2本体、SH-35 解除対象拡張、カスタム Window、個別チェックボックス UI は実装していない
- コード動作・session 形式・保存形式・schema・wrapper 変更なし

## v2.16.23 — TD-73: 静的テスト持続可能性ガイドライン

- review3 / review4 の指摘を受けて、docs-contract test / ソーステキスト静的確認テストの持続可能性ガイドラインを追加した
- docs-contract test では、文言完全一致ではなく重要語句確認・見出しアンカー・範囲解釈 helper を優先する方針を記録した
- ソーステキスト静的確認は、挙動テストや UI 非依存 helper の単体テストが難しい場合の補助手段として使う方針を記録した
- CRLF 依存、bare ID 検索、backlog 範囲表記の完全一致など、壊れやすいパターンを避ける方針を記録した
- コード動作・session 形式・保存形式・schema・wrapper 変更なし
...
- Tabs[].WorkspaceKind は復元時の最終判定を省略するための信頼ソースではなく、将来の選択的復元 UI 等でファイル読込前に種別を表示するためのヒントとして扱う
- 実際の読込時は、ファイル内容・拡張子・.nestsuite wrapper など既存の安全な判定を優先する
- 現行挙動をコメント・docs・テストで固定した
- LT-9 選択的 session 復元 UI は実装していない
- session 形式・保存形式・schema・wrapper 変更なし

## v2.16.15 — TD-67: 複数ファイルオープン失敗通知の改善

- 初回エキスパートレビュー R-7 への対応として、複数ファイルをまとめて開いたときの失敗通知を改善した
- 一部または複数のファイルを開けなかった場合、失敗件数・ファイル名・理由を表示するようにした
- 成功したファイルは従来どおり開き、失敗したファイルだけを通知する
- 表示が長くなりすぎないよう、一定件数を超える場合は「ほか N 件」とまとめる
- 自動復元・自動変換・.bak 自動適用はしていない
- 保存形式 / schema / wrapper / session 変更なし
...
- **本文エディタを折り返しなしにし、長い行は横スクロールで扱うようにした。** `EditorBox` を `TextWrapping=NoWrap`、`HorizontalScrollBarVisibility=Auto` に変更し、Markdown 表・長い URL・長い見出しなどは水平スクロールで読む動作にした。縦スクロールの行番号同期は維持し、横スクロールは行番号側へ同期しない。
- **行番号表示、現在行ハイライト、マーカー行ハイライトを維持している。** v2.15.4 で追加した `EditorBox.GetRectFromCharacterIndex` による現在行ハイライトの実描画位置基準も維持している。
- **保存形式 / schema / wrapper / session 変更なし。** NoteNest schema `1.4.2`、`.nestsuite` wrapper `formatVersion` `1.0`、Workspace 保存形式、session 形式はいずれも変更していない。外部依存追加なし。net48_test 再開なし。
...
- **`docs/archive/migrations/repository-rename.md` に `[完了済み]` 注記を追加した（v2.0.1 で完了済みのリポジトリ名変更手順）。**
...
- **v2.0.1 リリース後に GitHub リポジトリ名を `notenest` → `nestsuite` へ変更予定。** 変更後の確認手順を `docs/archive/migrations/repository-rename.md` に追加した。
...
- ファイル移動に伴い `docs/design/design-decisions.md` / `docs/design/nestsuite-known-limitations.md` / `docs/guide/nestsuite-user-guide.md` / `docs/backlog.md` / `docs/archive/completed-designs/nestsuite-preparation.md` の相対リンク・パス参照を修正した。
