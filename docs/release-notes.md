...
- **TD-82-1: `workspace-file-extension-unification.md` の正本性を再評価した。** 完了済み移行履歴だけでなく、1タブ1ファイル、`.nestsuite` wrapper `formatVersion: 1.0`、`workspaceKind`、wrapper / payload schema 分離、legacy 拡張子互換など現行仕様の補助正本を含むため、分類を Active Reference へ戻した。
- `docs/archive/migrations/` から `docs/development/workspace-file-extension-unification.md` へ、ファイル名を変えずに戻した。
- production コードコメント、schema policy、backlog、docs README、docs 棚卸し結果の参照を development 側の新パスへ更新した。`docs/archive/migrations/README.md` から同文書を archive に含む案内を削除した。
- TD-59 未実装という当時の記述は、現在は v2.16.32〜v2.16.39（TD-59a〜TD-59b-5）で完了済みであることを最小補正した。
- 保存形式、wrapper、schema、legacy 互換の仕様自体は変更していない。production コードの処理変更なし（参照コメントのみ更新）、UI 変更なし、session.json 変更なし、draft format 変更なし。
