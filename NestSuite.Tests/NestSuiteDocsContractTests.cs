...
public class NestSuiteDocsContractTests
{
    ...
    public static IEnumerable<object[]> ReleaseNoteVersionAndIdRecords()
    {
        ...
        yield return new object[] { "v2.18.0", "TN-3", "TN-3" };
        ...
        yield return new object[] { "v2.18.13", "TD-84", "TD-84" };
        // ID-6 は v2.18.2/v2.18.8/v2.18.13 等の release notes 内で「今回は実装していない」対象として
        // 既に言及されているため、見出し専用の文字列で確認する。
        yield return new object[] { "v2.18.14", "ID-6", "v2.18.14 — ID-6" };
        yield return new object[] { "v2.18.15", "SH-39", "SH-39" };
        ...
        yield return new object[] { "v2.18.20", "TD-86", "TD-86" };
        // M18 は過去の release notes 内で「状態は変更していない」対象として複数回言及されているため、
        // 見出し専用の文字列で確認する。
        ...
        yield return new object[] { "v2.19.7", "TD-83", "v2.19.7 — TD-83" };
        ...
        yield return new object[] { "v2.22.0", "LK-3", "v2.22.0 — LK-3" };
        yield return new object[] { "v2.23.0", "LK-2", "v2.23.0 — LK-2" };
        yield return new object[] { "v2.24.0", "TD-93", "v2.24.0 — TD-93" };
    ...
    public static IEnumerable<object[]> BacklogCompletedOpenItemAbsenceRecords()
    {
        ...
        yield return new object[] { "TD-90" };
        yield return new object[] { "SH-43" };
        ...
        // ── v2.19.7 TD-83: 完了済みeditor設計・旧入口READMEのarchive/削除候補再判定（欠番）──
        yield return new object[] { "TD-83" };
        // ── v2.19.9 SH-44: Shell横断検索のEscapeクローズ・フォーカス復帰（欠番）──
        ...
        // ── v2.20.0 TD-91: キーボード・アクセシビリティ横断レビュー総点検・回帰確認（欠番）──
        yield return new object[] { "TD-91" };
        // ── v2.20.1 TD-92: Workspace 間手動転送の共通ヘルパー設計（設計 version、欠番）──
        ...
        // ── v2.22.0 LK-3: TempNestスロット → IdeaNestカード追加（欠番）──
        yield return new object[] { "LK-3" };
        // ── v2.23.0 LK-2: TempNestスロット → 既存NoteNestタブへノート追加（欠番）──
        yield return new object[] { "LK-2" };
        // ── v2.24.0 TD-93: Workspace間手動転送の総点検・回帰確認（欠番）──
        yield return new object[] { "TD-93" };
    }
    ...
    {
        // REV7-1 / REV7-2 の追跡項目（SH-38 / L25）は v2.18.5 で、L24 は v2.18.7 で
        // それぞれ完了済み欠番となった（Backlog_DoesNotContainCompletedIdAsOpenItem で別途確認）。
        // ここでは、その対応が SH-35 等の無関係な既存項目を変化させていないことを確認する。
        var backlog = TestPaths.ReadBacklog();
        Assert.Contains("| SH-35 |", backlog);
    }
