...
public static class FileErrorMessages
{
    ...
    public static string ForLoad(Exception ex, string? path) => ex switch
    {
        ...
        // .txt の文字コードを安全に判定できない場合。元ファイルは変更していない。
    ...
    /// <summary>
    /// WorkspaceKind 判定失敗時のユーザー向けメッセージを返す。
    /// 「壊れています」と断定せず、理由に応じて文言を出し分ける。
    /// SchemaVersionTooNew は <see cref="SchemaVersionTooNewException"/> と同じ文言方針。
    /// </summary>
    ...
    /// <summary>
    /// path が分かる場合、InvalidFormat（`.nestsuite`
    ...
    /// <summary>
    /// 複数件通知（<see cref="MultipleOpenFailureMessageBuilder"/>・
