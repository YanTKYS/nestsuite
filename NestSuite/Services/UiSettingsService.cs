...
public class UiSettings
{
    ...
    /// <summary>
    /// NoteNest 左ペインのノート一覧表示順。Workspace ファイルへは保存しない、
...
public class UiSettingsService
{
    ...
    /// <summary>NoteNest 本文エディタで選択可能なフォント種類（端末非依存の主要候補に限定）。</summary>
    ...
    /// <summary>
    /// 未設定・空文字・候補外（削除されたフォント名の残存等）の場合は既定へフォールバックする。
    ...
    /// <summary>不正・未知の値は既定の作成順へ正規化する。</summary>
    ...
    /// <summary>
    /// <paramref name="dataPath"/> は読込失敗時の破損ファイル退避テスト用。
    /// 省略時は <c>%APPDATA%\NoteNest\ui-settings.json</c> を使う。
    /// </summary>
    ...
    public void Save(UiSettings settings)
    {
        try
        {
            // tmp 経由の atomic write 化。File.WriteAllText の既定エンコーディング
...
/// <summary><see cref="UiSettingsService.LoadWithRecovery"/> の結果。</summary>
