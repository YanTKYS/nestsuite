...
/// <summary>
/// Open ダイアログ・起動引数・pipe・最近ファイル・session 復元で共有できる
...
public static class ShellFileOpenPlanner
{
    ...
    /// すべての経路がこの prepared context 経路を使う。
    ///
        ...
        Func<string, (bool Success, WorkspaceFileOpenContext? Context, WorkspaceKindDetectionFailure Failure)>? prepareOpen = null)
    {
        ...
        // TryPrepareOpen 内でも .nestsuite の存在確認は行われるが、ファイル内容を二重に読むわけではない。
        // TOCTOU・failure 分類を変えないことを優先し、ここでの事前確認は省略しない。
        if (!fileExists(path))
        {
        ...
        // prepareOpen delegate の誤実装（テスト用シームの契約違反）を早期に検出する。
