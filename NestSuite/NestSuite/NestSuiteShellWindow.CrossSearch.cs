...
public partial class NestSuiteShellWindow
{
    ...
    // 横断検索を開く直前にフォーカスされていた要素。開いている間だけ保持し、
    ...
    /// <summary>
    /// 横断検索パネルを閉じる共通処理。×ボタン・Escapeのどちらから呼ばれても
    ...
    {
        // 二重クローズ防止。既に閉じている場合は何もせず、古い復帰先も破棄する。
        if (CrossSearchPanel.Visibility != Visibility.Visible)
        {
    ...
    /// <summary>
    /// Escapeが横断検索パネル内で押された場合だけ処理する。パネル外や非表示中のEscapeは
    ...
    /// <summary>
    /// 通常クローズ後のフォーカス復帰。Visibility変更と同一スタック内のFocus()は
    ...
    /// <summary>
    /// 現在アクティブなWorkspaceが提供する既定フォーカス先。既存のフォーカス復帰API
    ...
    /// （タブ一覧）へ進む。Workspace種別switchを新たに増やさない。
    /// </summary>
    ...
    /// <summary>
    /// 破棄済み・非表示・無効・フォーカス不可な要素へ無理にFocus()しない。
    ...
    /// 未オープンrecent filesの結果は、最近使ったファイルメニューと共有する
