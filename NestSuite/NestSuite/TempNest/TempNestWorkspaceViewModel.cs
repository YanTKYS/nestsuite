...
public class TempNestWorkspaceViewModel : BaseViewModel, IDisposable
{
    ...
    /// <summary>recent filesリンクをクリックしたときの実処理。Shellが配線する（PromoteRequestedと同じ委譲パターン）。</summary>
    public Action<string>? OpenContinueFromRecentRequested { get; set; }

    /// <summary>起動時にShellから渡されたrecent files上位項目（既定は空）。</summary>
    ...
    /// <summary>
    /// 「続きから」を表示すべきか。recent filesまたは保持中draftが1件以上あり、
    ...
    /// <summary>
    /// 起動時に一度だけ、recent files上位項目と保持中draft件数を設定する。
    ...
    /// <summary>
    /// recentリンククリック時にファイル不存在等で最近使ったファイル一覧から
    ...
    /// <summary>
    /// 同一起動中に通常タブが1枚でも追加されたら「続きから」を再表示しないようにする、
    /// 単発ラッチ（<see cref="MarkGettingStartedHintDismissed"/>と同じパターン、
    ...
    /// <summary>
    /// 各スロットのタイトル欄・本文欄に適用する Workspace 共通フォント種類。
    ...
    {
        // スロットへの入力・クリアいずれも「利用者がここで作業を始めた」印であり、
