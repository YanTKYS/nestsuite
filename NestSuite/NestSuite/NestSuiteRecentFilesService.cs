...
public class NestSuiteRecentFilesService
{
    ...
    /// 失敗時は空履歴を返し起動は継続するが、原因を ErrorLog（Error のみ）に記録し、
    /// 可能であれば破損ファイルを <c>.corrupt</c> へ退避する（session の復旧と同型）。
    /// 退避に失敗しても起動は妨げない。
    /// </summary>
    ...
    /// <summary>
    /// 破損した nestsuite-recent-files.json を <c>.corrupt</c>（既存なら日時付き）へ退避する。
